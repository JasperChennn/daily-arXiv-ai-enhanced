...
TL;DR: 本文提出SKETCHGAIT框架，将草图作为步态识别的新视觉模态，通过解耦的多模态架构结合标签引导的解析和标签自由的草图表示，显著提升识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要依赖轮廓和解析两种表示方法。轮廓表示稀疏且缺乏内部结构细节，限制了区分能力；解析表示虽然丰富了结构信息，但严重依赖上游人体解析器，导致跨数据集性能不稳定，有时甚至不如轮廓表示。需要探索新的步态表示范式。

Method: 从结构视角重新审视步态表示，定义由边缘密度和监督形式构成的设计空间。提出草图作为新的视觉模态，通过边缘检测器从RGB图像中提取高频结构线索（如肢体关节和自遮挡轮廓），无需语义标签。进一步提出SKETCHGAIT框架，包含两个独立的模态特定学习流和一个轻量级早期融合分支，实现层次化解耦的多模态学习。

Result: 在SUSTech1K和CCPG数据集上的实验验证：SKETCHGAIT在SUSTech1K上达到92.9%的Rank-1准确率，在CCPG上达到93.1%的平均Rank-1准确率，证明了所提模态和框架的有效性。

Conclusion: 草图作为步态识别的新视觉模态具有重要价值，标签引导的解析和标签自由的草图在语义上解耦且在结构上互补。提出的SKETCHGAIT框架通过层次化解耦的多模态学习有效利用这种互补性，显著提升了步态识别性能。
...
TL;DR: DreamCAD是一个多模态生成框架，无需CAD特定标注，直接从点级监督生成可编辑的BReps，并创建了最大的CAD标注数据集CADCap-1M


<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于小型标注数据集，而数百万未标注的3D网格未被利用，限制了可扩展CAD生成的发展

Method: 将BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，通过点级监督进行大规模训练，同时重建连接且可编辑的表面

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点模态上均达到最先进性能，几何保真度提升，用户偏好超过75%

Conclusion: DreamCAD通过无标注训练和可微分方法实现了可扩展的CAD生成，同时发布的CADCap-1M数据集将推动文本到CAD研究
...
TL;DR: ABRA是一种对抗性批量表示增强方法，通过参数化特征统计量作为结构化不确定性，在表示空间中合成最坏情况的生物批次扰动，以解决细胞成像中的生物批次效应问题。
...
Method: 将生物批次缓解问题框架化为域泛化问题，提出ABRA方法：1) 通过参数化特征统计量作为结构化不确定性来显式建模批次间统计波动；2) 采用最小-最大优化框架，在表示空间中主动合成最坏情况的生物批次扰动；3) 引入严格的角度几何边界来保持细粒度类别可区分性；4) 为防止对抗探索中的表示崩溃，引入协同分布对齐目标。

Result: 在RxRx1和RxRx1-WILDS大规模基准测试上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批量表示增强有效缓解了细胞成像中的生物批次效应，将问题重新框架化为域泛化问题，通过结构化不确定性建模和对抗扰动合成实现了对未见生物批次的更好泛化。
...
TL;DR: 提出PFS模块，通过稳定BEV特征统计、抑制传感器退化区域和残差校正，提升现有相机-LiDAR融合检测器在域偏移和传感器故障下的鲁棒性，无需修改原架构或重新训练。
...
Method: 提出后融合稳定器(PFS)，作为轻量级模块操作于现有检测器的中间BEV表示，通过稳定特征统计、抑制传感器退化空间区域和自适应残差校正来生成精炼特征图。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，仅需3.3M参数。

Conclusion: PFS作为近恒等变换设计，在保持性能的同时显著提升现有检测器在相机和LiDAR损坏下的鲁棒性，易于集成到已部署系统中。
...
TL;DR: CBM-Suite是一个系统性解决概念瓶颈模型四大挑战的框架：提出概念集适用性评估指标、解决线性化问题、缩小准确率差距、分析不同视觉骨干和视觉语言模型的影响。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型虽然基于人类可理解的概念进行预测，但面临四个基本限制：缺乏预评估概念相关性的指标、线性化问题导致模型绕过概念瓶颈、与不透明模型相比存在准确率差距、缺乏对不同视觉骨干和视觉语言模型影响的系统性研究。

Method: 1. 提出基于熵的指标来量化概念集对特定数据集的固有适用性；2. 在概念激活和分类器之间插入非线性层来解决线性化问题；3. 使用线性教师探针引导的蒸馏损失来缩小准确率差距；4. 系统分析不同视觉编码器、视觉语言模型和概念集如何影响CBM的准确率和可解释性。

Result: 广泛的评估表明，CBM-Suite能够产生更准确的模型，并为改进基于概念的可解释性提供了见解。

Conclusion: CBM-Suite是一个方法论框架，系统性解决了概念瓶颈模型的关键挑战，通过量化概念适用性、确保概念相关性、提升模型性能以及提供系统性分析，推动了基于概念的可解释性研究。
...
Method: 对数据集每个元素，在潜在空间中检索其最近邻，插值潜在表示后解码，计算解码样本与原数据集的FID

Result: iFID与扩散gFID呈现强相关性（Pearson和Spearman相关系数约0.85），是首个展示此强相关的指标

Conclusion: iFID能有效预测扩散模型生成质量，rFID与扩散细化阶段质量相关，而iFID与扩散导航阶段质量相关，为评估扩散模型提供了新工具
...
TL;DR: 本文提出隐式错误计数(IEC)方法，用于解决缺乏理想参考答案的强化学习任务，通过枚举和加权错误而非构建评分标准来生成可靠奖励信号。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的强化学习方法(如RaR)依赖理想参考答案来构建评估标准，但许多现实任务存在多个有效输出且缺乏单一理想答案。本文识别出这种无参考设置是当前后训练方法的空白，需要新的解决方案。

Method: 提出隐式错误计数(IEC)方法：1)枚举响应中的错误而非检查正确部分；2)在任务相关维度上应用严重性加权评分；3)通过隐式分数发射和组校准将错误转换为校准后的每维度奖励。还提出级联错误计数(CEC)作为评估指标，并创建Mismatch-DressCode基准测试。

Result: 在虚拟试穿(VTO)任务中，IEC在MDressBench基准上全面优于RaR(CEC分数：5.31 vs 5.60在平面参考；5.20 vs 5.53在非平面)。在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越6个基线方法。CEC指标与人类偏好高度相关(60% top-1 vs 30%其他)。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号。隐式分数发射和组校准是使错误计数成为可靠奖励的必要设计选择，IEC方法填补了无参考设置下的强化学习空白。
...
TL;DR: SemVID是一种无需训练的视频时序定位剪枝框架，通过保留查询关键证据和跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4%的mIoU，实现5.8倍预填充加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位需要处理长未修剪视频，使用视频语言模型成本过高。现有视觉token剪枝方法直接应用于VTG会导致性能严重下降，因为VTG依赖于边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG特定剪枝原则：证据保留（保持查询关键补丁，特别是事件边界周围）和连接强度（保持token级跨帧连接性用于长程证据聚合）。首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三类token：对象token（多样化查询关键证据）、运动token（捕捉有意义的转换并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在VTG基准测试中，SemVID在仅使用12.5%视觉token的情况下保持95.4%的mIoU，实现5.8倍预填充加速，在相同预算下持续优于先前方法。

Conclusion: SemVID通过识别VTG特定剪枝原则并构建具有互补语义角色的紧凑而连贯的token子集，实现了强大的准确性-效率权衡，为高效视频时序定位提供了有效解决方案。
...
TL;DR: OWL是一种基于视觉运动线索的3D感知函数，通过局部视觉膨胀和刚性旋转两种运动线索，直接从原始视觉运动图像序列中获取3D感知信息，无需环境先验知识或相机运动参数。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，提供一种无需先验环境知识、相机运动参数或静止环境假设的统一分析方法，填补理论概念与机器人、自主导航实际应用之间的差距。

Method: 基于两种基本视觉运动线索：1) 注视点附近点的感知局部视觉膨胀，2) 刚性物体相对于注视点的感知旋转。通过像素级并行计算直接从原始视觉运动图像序列中获取OWL值，无需测量相对瞬时方向范围和方向平移等物理量。

Result: 仿真实验表明OWL实现了3D物体随时间变化的几何恒常性，仅凭视觉运动线索即可实现缩放3D场景重建。该方法能够获得无需先验知识的3D感知值，为相对运动中的3D点提供实时替代表示。

Conclusion: OWL提供了一种统一的、基于时间的分析方法，增强和简化了关键感知能力，可作为下一代自主系统的构建模块，为机器感知提供新视角，对自然感知、行为心理学和神经功能理解也有潜在贡献。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理能力的基准，包含超过46,000个多模态检索候选项和747个开放可验证问题，揭示当前MLLMs在多模态检索方面的显著瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要评估单模态理解能力，但忽略了现实应用中需要从大规模异构多模态语料库中检索相关证据的关键需求。大多数基准将检索限制在小规模、单模态候选集中，过度简化了搜索空间并夸大了端到端可靠性。

Method: 提出MultiHaystack基准，包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的已验证证据项，要求跨模态的证据定位和细粒度推理。

Result: 模型在提供相应证据时表现良好，但从完整语料库中检索证据时性能急剧下降。最强的检索器E5-V仅达到40.8%的Recall@1，而最先进的MLLMs如GPT-5在提供相应证据时推理准确率为80.86%，但在top-5检索下降至51.4%。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基于图像的定位方面有所进展，但精确的全球地理定位仍面临挑战，主要因为视觉景观的固有模糊性和听觉线索的未充分利用。
...
Conclusion: 视听地理定位框架通过可解释的声学感知和多模态推理，有效解决了地理定位中的模糊性问题，为高精度全球定位提供了新方法。
...
TL;DR: 本文提出了一种自动化流程，将ImageNet训练集转换为多标签数据集，无需人工标注，通过自监督视觉Transformer进行无监督物体发现，生成高质量多标签标注，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准采用单标签假设，但许多图像包含多个物体，导致标签噪声并限制学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，使模型学习更丰富、更鲁棒的表征。尽管先前工作改进了验证集，但训练集仍缺乏可扩展的高质量多标签标注。

Method: 提出自动化流程：1) 使用自监督视觉Transformer进行无监督物体发现；2) 选择与原始标签对齐的区域训练轻量级分类器；3) 将该分类器应用于所有区域，生成整个数据集的一致多标签标注。整个过程无需人工标注。

Result: 生成的多标签标注在定性评估中与人类判断高度一致。相比传统单标签方案，使用多标签监督训练的模型在多个架构上获得一致的域内准确率提升（ReaL上最高+2.0 top-1准确率，ImageNet-V2上+1.5）。在下游任务迁移性方面表现更强（COCO上最高+4.2 mAP，VOC上+2.3 mAP）。

Conclusion: 准确的多标签标注对于提升分类性能和表征学习至关重要。提出的自动化流程成功生成了高质量的多标签ImageNet训练集，显著改善了模型性能，证明了多标签监督的价值。项目代码和生成的多标签标注已开源。
...
<details>
  <summary>Details</summary>
Motivation: 当前手术时间线创建方法存在缺陷：术后报告通常模糊不清，而手动标注视频又极其耗时。需要一种折中方案来自动化生成结构化手术时间线

Method: 采用CLIP多模态框架，将手术视频帧与文本手势描述对齐。使用CLIP视觉编码器提取视频帧表示，文本编码器嵌入手势句子到共享嵌入空间，通过微调改进视频手势与文本标记的对齐。训练后模型可预测视频帧的手势和阶段，构建结构化手术时间线

Result: 该方法能够自动创建手术时间线和叙述，利用预训练多模态表示桥接视觉手势和文本叙述，显著减少外科医生手动视频审查和标注的需求

Conclusion: 提出的CLIP多模态框架为手术视频分析提供了一种有效的自动化解决方案，通过视觉-文本对齐实现手术时间线的自动生成，平衡了术后报告的模糊性和手动标注的耗时性
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕获户外图像，通过条件生成环境贴图，实现用户控制的太阳和云层定位以及大气纹理控制。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习的天空模型在重现准确和逼真的自然天空方面存在不足，特别是在高分辨率下难以准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输、阴影和色调失真。

Method: 提出Icarus模型，能够学习全动态范围物理捕获户外图像的曝光范围，支持条件生成环境贴图，允许用户直观定位太阳和云层，并扩展当前最先进技术以实现用户控制的大气纹理。

Result: Icarus可与全动态范围物理捕获户外图像或参数化天空模型互换，在基于图像的照明中提供前所未有的准确性、真实感、光照方向性（阴影）和色调。

Conclusion: Icarus是一个能够生成高质量、逼真天空环境贴图的全天气天空模型，解决了现有深度学习天空模型在高分辨率太阳区域建模方面的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有区域指令布局控制方法存在两个主要问题：1）基于训练的方法继承数据偏差且会降低图像质量；2）当前技术难以处理遮挡顺序，限制了实际应用。研究发现空间布局和遮挡在去噪早期阶段就已确定，重新排列早期潜在结构足以修改最终输出

Method: 采用两阶段方案：1）实例初始化阶段：利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤根据层顺序融合形成具有预设布局的统一潜在表示；2）语义护理阶段：通过分层注意力增强强化区域细节并保持遮挡顺序，使用序列化分层注意力路径与标准全局路径并行，更新在层透明度调度器下组合

Result: LayerBind在定性和定量评估中均表现出有效性，能够实现精确的区域控制和遮挡顺序管理，支持可编辑工作流程，如更改实例或重新排列可见顺序，在创意应用中展现出强大潜力

Conclusion: LayerBind是一种无需训练、即插即用的解决方案，通过分层建模和绑定机制解决了文本到图像生成中的区域布局控制和遮挡顺序问题，为创意应用提供了灵活强大的工具
...
TL;DR: BM25-V：基于视觉词稀疏表示的图像检索方法，使用SAE提取视觉词，结合BM25评分实现高效检索，可作为密集重排的前置检索器


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确但可解释性差、计算成本高，需要一种高效且可解释的检索方法作为前置检索器

Method: 使用稀疏自编码器（SAE）从Vision Transformer补丁特征中提取稀疏视觉词激活，应用Okapi BM25评分进行检索，利用倒排索引实现高效检索

Result: 在7个基准测试中，Recall@200 ≥ 0.993，两阶段检索管道（BM25-V检索+密集重排）仅需重排200个候选，平均准确率与密集检索相差仅0.2%

Conclusion: BM25-V是一种高效、可解释的图像检索方法，可作为密集重排的前置检索器，在保持高召回率的同时显著降低计算成本，且具有零样本迁移能力
...
TL;DR: 提出LIPAR框架，通过检测和跳过重复的潜在补丁来减少视频生成计算延迟，引入注意力恢复机制保持生成质量，实现1.45倍吞吐量提升
...
Conclusion: LIPAR框架有效利用视频时间冗余性，在保持生成质量的同时显著提升计算效率，弥合了传统压缩算法与现代生成管道之间的差距
...
TL;DR: Cog2Gen3D：一个3D认知引导的扩散框架，通过语义和几何表示的双流融合来解决3D生成中的尺度不一致问题，实现物理合理且结构合理的3D生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法主要依赖几何特征作为条件，但只能建模相对关系，容易产生绝对几何的尺度不一致问题。作者认为语义信息和绝对几何共同赋能3D认知，从而实现物理世界中可控的3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：将不同模态编码为语义和几何表示，并提取逻辑表示；2）3D潜在认知图：将不同表示结构化为双流语义-几何图，通过基于共通的交叉注意力融合得到3D认知图；3）认知引导的潜在扩散：利用融合的3D认知图作为条件，引导潜在扩散过程生成3D高斯表示。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，结果表明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: 该统一框架通过3D认知图确保3D生成的物理合理性和结构合理性，为解决3D生成中的尺度不一致问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的根本权衡：2D方法因过度裁剪而受限，3D技术则因脆弱的优化流程在极端运动下失效。需要一种能确保全场景可靠性的解决方案。

Method: VS3R框架协同前馈3D重建与生成式视频扩散：联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；采用双流视频扩散模型通过结合结构引导与语义锚点来修复遮挡区域和校正伪影。

Result: VS3R在不同相机模型上实现了高保真、全帧稳定，在鲁棒性和视觉质量方面显著优于最先进的方法。

Conclusion: VS3R通过协同3D重建与生成式视频扩散，成功解决了视频稳定中的几何鲁棒性与全帧一致性权衡问题，为全场景可靠的高质量视频稳定提供了有效解决方案。
...
TL;DR: TumorChain：一种用于临床肿瘤分析的多模态推理框架，通过链式思维推理从CT影像到病理预测，提高诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 临床放射学和精准肿瘤学需要准确的肿瘤分析，包括早期检测、病变表征和病理级风险评估。链式思维推理能够实现从影像发现到临床印象再到病理结论的逐步解释，提高可追溯性并减少诊断错误。

Method: 构建TumorCoT数据集（150万条CoT标注的VQA指令配对的3D CT扫描），提出TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理实现证据基础、结论聚合和病理预测。

Result: 在病变检测、印象生成和病理分类方面相比强基线模型有持续改进，在DeepTumorVQA基准测试上表现出强大的泛化能力。

Conclusion: 多模态推理在临床实践中具有实现可靠、可解释肿瘤分析的潜力，TumorChain框架通过链式思维推理提高了诊断的准确性和可追溯性。
...
TL;DR: PatchCue：一种基于图像块的视觉提示范式，通过将图像分割为块并在块级别表示视觉提示，显著提升视觉语言模型的视觉推理能力，优于像素级边界框和点级提示。


<details>
  <summary>Details</summary>
Motivation: 现有推理范式（如Chain-of-Thought）仅依赖文本信息，未能充分利用重要视觉线索。先前工作虽然引入了像素级视觉提示，但需要精确的空间定位，增加了学习复杂度。需要一种更符合人类感知习惯且与现代VLM的块标记化输入对齐的视觉提示方法。

Method: 提出PatchCue：基于块的视觉提示范式。将图像分割为块，在块级别表示视觉提示。采用两阶段训练方法：1）冷启动监督微调，使模型输出块级提示；2）强化学习，使用过程监督的提示奖励来引导中间视觉推理步骤。

Result: 在多个VLM和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue能持续提升整体模型性能。块级提示在性能上优于像素级边界框和点级提示。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉提示更好地利用视觉信息，增强了视觉语言模型的推理能力。
...
TL;DR: 该论文系统评估了合成新视角在视频地点识别(VPR)中的应用，通过五个公共VPR图像数据库和七种典型图像相似性方法，发现小规模合成添加能改善VPR识别统计，而大规模添加时视角变化幅度不如添加视图数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角生成在机器人导航中有多种潜在应用价值：在基于图像的导航中，地面机器人拍摄场景生成的合成俯视图可用于引导空中机器人到达该位置；在视频地点识别中，从空中生成的地面位置新视角可以帮助无人机识别地面机器人见过的地点，反之亦然。

Method: 使用五个公共VPR图像数据库和七种典型图像相似性方法，对合成新视角在VPR中的应用进行系统评估。研究分析了不同规模合成添加对识别性能的影响。

Result: 对于小规模合成添加，新视角能改善VPR识别统计；对于大规模添加，视角变化幅度的重要性不如添加视图数量和数据集图像类型重要。

Conclusion: 合成新视角在VPR中具有实用价值，特别是在小规模添加时能提升识别性能。研究结果为合成视图在机器人导航和地点识别中的有效应用提供了指导。
...
TL;DR: CylinderSplat：一种用于全景3D高斯泼溅的前馈框架，通过圆柱形Triplane表示和双分支架构解决稀疏视图下的遮挡问题，实现高质量全景新视角合成。
...
Result: 广泛实验表明，CylinderSplat在单视图和多视图全景新视角合成中均达到最先进水平，在重建质量和几何精度方面优于先前方法。

Conclusion: CylinderSplat通过圆柱形Triplane表示和双分支架构，有效解决了全景3D高斯泼溅中的几何表示和遮挡问题，实现了高质量的全景新视角合成。
...
TL;DR: 该论文提出了CORE-Seg框架，通过语义引导提示适配器将推理与分割相结合，用于复杂病变分割，在ComLesion-14K基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割正从传统的视觉模式匹配转向认知推理分析。现有通用多模态大语言模型具有广泛常识但缺乏复杂病变所需的专业视觉推理能力，而传统分割模型擅长像素级分割但缺乏逻辑可解释性。

Method: 提出CORE-Seg端到端框架，通过语义引导提示适配器整合推理与分割。采用从SFT到GRPO的渐进训练策略，配备自适应双粒度奖励机制以缓解奖励稀疏性问题。

Result: 在ComLesion-14K基准上取得37.06%的平均Dice分数（比第二名基线高14.89%），失败率降低至18.42%，达到最先进水平。

Conclusion: CORE-Seg框架成功将推理能力整合到医学图像分割中，为复杂病变分割提供了新的解决方案，在性能和可解释性方面均有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式有向距离场和事后布局优化，缺乏统一的场景重建框架。PixARMesh旨在直接从单张图像生成完整、连贯的3D室内场景网格，满足下游应用需求。
...
Conclusion: PixARMesh提供了一种从单张RGB图像自回归重建完整3D室内场景网格的有效方法，通过统一模型联合优化布局和几何，生成可直接使用的艺术家级网格，在重建质量和实用性方面均表现出色。
...
TL;DR: BlackMirror是一个用于黑盒场景下检测文本到图像模型后门的框架，通过分析生成图像中部分语义模式的稳定操纵来识别后门攻击，无需训练即可部署。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像相似度的后门检测方法假设后门触发的生成具有强一致性，但无法应对新兴的视觉多样性后门攻击。研究发现后门攻击中只有部分语义模式被稳定操纵，其余内容保持多样或良性。
...
Conclusion: BlackMirror为黑盒文本到图像模型后门检测提供了一个有效的解决方案，特别适用于处理视觉多样性的新型后门攻击，可作为MaaS应用中的即插即用模块。
...
TL;DR: InnoAds-Composer：单阶段三条件控制的产品海报生成框架，通过重要性分析和路由机制优化计算效率，结合文本特征增强模块提升中文文本渲染质量，并在首个包含主体、文本和样式条件的数据集上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常采用多阶段流程，难以同时控制主体、文本和样式，且存在主体保真度差、文本不准确、样式不一致等问题。需要开发能够高效实现三条件控制的单阶段框架。

Method: 提出InnoAds-Composer单阶段框架：1) 通过重要性分析确定各条件对层和时间步的响应位置，仅将条件路由到最敏感位置，缩短有效token序列以降低计算开销；2) 设计文本特征增强模块(TFEM)，融合字形图像和字形裁剪特征以提升中文文本渲染准确性；3) 构建首个同时包含主体、文本和样式条件的高质量电商产品海报数据集。

Result: 实验表明InnoAds-Composer在显著优于现有产品海报生成方法的同时，未明显增加推理延迟。在主体保真度、文本准确性和样式一致性方面均有明显提升。

Conclusion: InnoAds-Composer通过创新的三条件控制机制和文本特征增强模块，有效解决了多阶段方法存在的问题，为电商产品海报生成提供了高效、准确的单阶段解决方案，并在首个综合条件数据集上验证了其优越性。
...
TL;DR: RAC是一种基于Rectified Flow的自动编码器，通过多步解码、双向推理和生成式解码提升性能，在重建和生成质量上超越SOTA VAE，计算成本降低约70%


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径不够灵活，且需要独立的编码器和解码器导致参数冗余。RAC旨在通过Rectified Flow的流形学习框架来解决这些问题，实现更高效的生成模型。

Method: RAC采用Rectified Flow框架：1) 通过将解码器应用于流时间步实现多步解码，解码路径直线且可修正；2) 支持双向推理，解码器通过时间反转可作为编码器使用，减少41%参数；3) 生成式解码方法允许在路径上修正潜在变量。

Result: 实验表明RAC在重建和生成质量上均超越最先进的VAE模型，同时计算成本降低约70%。模型通过可修正的解码路径部分解决了重建-生成差距问题。

Conclusion: RAC成功地将Rectified Flow框架应用于自动编码器设计，通过多步解码、双向推理和生成式解码机制，在保持高质量重建和生成的同时显著降低了计算成本和参数数量，为生成模型提供了新的设计思路。
...
TL;DR: 该论文提出三种偏置缓解技术来改善概念瓶颈模型（CBM）的公平性：top-k概念过滤减少信息泄漏、移除偏置概念、对抗性去偏置，在公平性-性能权衡方面优于先前工作。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型（CBMs）理论上应通过将图像映射到人类可解释概念来增强公平性，但实际中概念会泄漏与语义无关的信息，且在ImSitu等数据集上仅能边际减少性别偏置，需要更有效的公平性改进方法。

Method: 提出三种偏置缓解技术：1. 使用top-k概念过滤器减少信息泄漏；2. 识别并移除偏置概念；3. 应用对抗性去偏置方法。这些方法共同改善CBM的公平性。

Result: 提出的去偏置CBM在公平性-性能权衡方面优于先前工作，表明该方法在实现公平且可解释的图像分类方面取得了显著进展。

Conclusion: 通过结合三种偏置缓解技术，去偏置的概念瓶颈模型为公平且可解释的图像分类提供了重要进展，在保持模型性能的同时显著改善了公平性。
...
Method: 提出CollabOD框架，集成三种策略：1) 结构细节保留，明确保留结构细节；2) 跨路径特征对齐，在跨尺度融合前对齐异构特征流；3) 定位感知轻量化设计。从图像处理、通道结构和轻量化设计三个角度优化传统无人机感知模型架构，并采用统一的细节感知检测头。

Result: 该设计在保持高效推理的同时增强了表示稳定性，统一的细节感知检测头进一步提高了回归鲁棒性，且不引入额外的部署开销。代码已开源。

Conclusion: CollabOD通过协同优化框架有效解决了无人机图像中小目标检测的关键问题，在保持轻量化的同时提升了检测性能和鲁棒性。
...
TL;DR: 该论文提出Art3D框架，将2D转3D的目标从物理精确的视差估计转向艺术连贯的视差合成，通过双路径架构分离全局深度参数和局部艺术效果，学习专业3D电影数据来提升艺术表现力。


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽然几何精度高，但艺术表现力不足，无法复制专业3D电影的沉浸感和情感共鸣体验。几何重建范式将艺术家有意的艺术意图（如策略性的零平面偏移用于弹出效果和局部深度雕刻）误认为是数据噪声或模糊性。
...
Conclusion: 论文主张从物理精确的视差估计转向艺术连贯的视差合成的新范式，Art3D框架通过分离全局和局部深度处理，学习专业电影数据，为开发具有艺术表现力的2D转3D工具提供了有前景的方向。
...
TL;DR: Pano3DComposer是一个高效的前馈框架，用于从全景图像生成3D场景，通过解耦对象生成和布局估计，在RTX 4090 GPU上约20秒内生成高保真3D场景。
...
Method: 提出Pano3DComposer框架，包含两个核心技术：1）Object-World Transformation Predictor（对齐VGGT架构），使用目标对象裁剪、多视角对象渲染和相机参数预测从局部到世界坐标的变换；2）Coarse-to-Fine对齐机制，通过场景渲染反馈迭代优化几何一致性。

Result: 在合成和真实数据集上的图像/文本到3D任务中实现了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景。

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，结合伪几何监督和粗到细对齐机制，实现了高效、高质量的全景图像到3D场景生成。
...
### [33] [Skeleton-to-Image Encoding: Enabling Skeleton Representation Learning via Vision-Pretrained Models](https://arxiv.org/abs/2603.05963)
*Siyuan Yang,Jun Liu,Hao Cheng,Chong Wang,Shijian Lu,Hedvig Kjellstrom,Weisi Lin,Alex C. Kot*

Main category: cs.CV

TL;DR: S2I将3D骨架序列转换为图像格式，使预训练视觉模型能用于自监督骨架表示学习，解决了骨架数据格式差异和数据集稀缺问题。
...
Result: 在NTU-60、NTU-120和PKU-MMD数据集上的实验表明，该方法在自监督骨架表示学习中具有有效性和泛化能力，包括在跨格式评估的挑战性场景下。

Conclusion: S2I首次实现了将预训练视觉模型用于骨架表示学习，为异构骨架数据提供了统一表示，成功将视觉域知识迁移到骨架分析中。
...
Method: 两阶段框架：第一阶段训练过程编码器，通过自动生成中间帧获取关键帧，并通过基于描述的条件掩码重建任务学习潜在动态；第二阶段将训练好的编码器集成到编码器-解码器模型中，引入可学习的过程查询来推断潜在过程表示，然后解码为文本描述，整个模型通过端到端的描述损失进行训练。

Result: 在三个数据集上的实验证明了ProCap的有效性，代码和预训练模型已开源。

Conclusion: ProCap通过将变化建模从静态图像比较重新定义为动态过程建模，能够更好地理解和描述变化过程，为变化描述任务提供了新的视角和方法。
...
TL;DR: 该报告探讨了巴基斯坦制造的手术器械及其缺陷检测，通过与行业领导者合作，开发了包含4,414张高分辨率图像的数据集，并采用YOLOv8、ResNet-152和EfficientNet-b4等深度学习架构进行自动化缺陷检测，旨在提升制造质量和手术安全。


<details>
  <summary>Details</summary>
Motivation: 现代医疗中手术器械的质量标准对临床成功至关重要。即使是微小缺陷也可能导致致命后果，因此制造精度极为重要。报告旨在解决手术器械制造缺陷的识别和纠正问题，防止经济损失并保障患者生命安全。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作
2. 开发包含4,414张高分辨率图像的新数据集
3. 采用深度学习架构进行自动化缺陷检测，包括YOLOv8、ResNet-152和EfficientNet-b4
4. 通过自动光学检测（AOI）工具进行质量保证

Result: 1. 建立了专门针对巴基斯坦制造手术器械缺陷分析的数据集
2. 实现了对裂纹、锈蚀和结构不规则等关键缺陷的识别和纠正
3. 为制造商、医疗专业人员和监管机构提供了有价值的资源
4. 推动了自动化缺陷检测方法的发展

Conclusion: 通过结合行业专业知识和尖端技术，该报告为提升手术器械标准做出了贡献，确保更可靠的医疗环境。自动化缺陷检测方法的集成显著提高了制造过程的质量标准，有助于预防潜在的安全风险和经济损失。
...
TL;DR: RAID是一个用于驾驶员风险感知研究的大规模数据集，包含4,691个标注视频片段，涵盖多样交通场景。作者提出了一种弱监督风险对象识别框架，在RAID和HDDS数据集上分别实现了20.6%和23.1%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞移动性是智能车辆系统的关键目标，这需要理解驾驶员的风险感知——一个复杂的认知过程，受到驾驶员对外部刺激的自愿反应以及周围道路使用者对自车的关注度的影响。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 提出了RAID数据集，包含4,691个标注视频片段，涵盖驾驶员意图操作、道路拓扑、风险情况（如横穿行人）、驾驶员反应和行人注意力等标签。基于此数据集，开发了一个弱监督风险对象识别框架，通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。

Result: 实验评估表明，该方法在RAID数据集上比现有最优方法性能提升20.6%，在HDDS数据集上提升23.1%。研究还分析了行人注意力在风险评估中的作用，并证明了所提数据集的价值。

Conclusion: RAID数据集为驾驶员风险感知和上下文风险评估研究提供了重要资源。提出的弱监督风险对象识别框架通过建模驾驶员意图与反应的关系，在风险识别方面取得了显著性能提升，为智能车辆系统的零碰撞目标提供了支持。
...
Method: 提出两阶段编码机制：跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据；ISTS编码提取历史观测的时序特征；自适应查询特征提取器压缩MLLMs学习到的token；多模态对齐模块缓解模态差异

Result: 在真实数据上的广泛实验证明了所提解决方案的有效性

Conclusion: MM-ISTS通过整合时间、视觉和文本模态，显著提升了不规则采样时间序列的预测性能
...
Method: 提出TAR-ViTPose，包含两个核心模块：1）联合中心时序聚合（JTA）：为每个关节点分配可学习的查询token，从相邻帧中选择性关注对应区域；2）全局恢复注意力（GRA）：将聚合的时序特征恢复回当前帧的token序列，丰富姿态表示同时保持全局上下文。

Result: 在PoseTrack2017基准测试中，相比单帧基线ViTPose获得+2.3 mAP提升，超越现有最先进的视频姿态估计方法，同时在实际应用中实现更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效的时序信息聚合与恢复机制，显著提升了视频姿态估计的鲁棒性和准确性，在保持高计算效率的同时解决了现有静态方法忽略时序连贯性的问题。
...
### [39] [Demystifying KAN for Vision Tasks: The RepKAN Approach](https://arxiv.org/abs/2603.06002)
*Minjong Cheon*
...
<details>
  <summary>Details</summary>
Motivation: 现有CNN和Transformer模型在遥感图像分类中通常作为不可解释的黑盒运行，缺乏对分类决策的物理可解释性，限制了其在需要透明推理的地球观测应用中的实用性。
...
Conclusion: RepKAN具有显著潜力作为未来可解释视觉基础模型的主干架构，为遥感图像分类提供了既高性能又具有物理可解释性的解决方案。
...
TL;DR: 提出一种前馈三角形基元生成框架，可从多视角图像直接预测连续三角形表面，实现免优化、免后处理的仿真就绪3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法依赖耗时的逐场景优化，难以实时部署；新兴前馈高斯泼溅方法缺乏显式流形几何，无法直接用于仿真。需要一种能生成仿真就绪模型的高效重建方法。
...
Conclusion: 该方法解决了现有方法对逐场景优化的依赖和缺乏显式几何的问题，实现了高效、仿真就绪的3D重建，为机器人和仿真应用提供了实用解决方案。
...
### [41] [OD-RASE: Ontology-Driven Risk Assessment and Safety Enhancement for Autonomous Driving](https://arxiv.org/abs/2603.05936)
*Kota Shimomura,Masaki Nambata,Atsuya Ishikawa,Ryota Mimura,Takayuki Kawabuchi,Takayoshi Yamashita,Koki Inoue*

Main category: cs.CV

TL;DR: OD-RASE框架通过本体驱动的数据过滤，结合大规模视觉语言模型和扩散模型，检测导致交通事故的道路结构并生成基础设施改进方案，以提升自动驾驶系统的安全性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍存在局限。现有道路基础设施为人类驾驶员设计，安全改进通常只在事故发生后进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成重大挑战。

Method: 1. 基于道路交通系统专业领域知识形式化本体；2. 使用大规模视觉语言模型生成基础设施改进方案；3. 通过本体驱动数据过滤增强可靠性；4. 在事故前道路图像上自动标注改进方案构建新数据集；5. 提出OD-RASE模型，结合LVLM和扩散模型生成改进方案及改进后道路环境的生成图像。

Result: 实验表明，本体驱动的数据过滤能够高度准确地预测导致事故的道路结构及相应的改进计划。该框架为构建更安全数据集和提升自动驾驶系统安全性提供了有效方法。

Conclusion: OD-RASE框架通过主动检测事故风险道路结构并生成改进方案，有助于提升整体交通环境安全性，是推动自动驾驶系统更广泛应用的重要一步。
...
### [42] [Probing Visual Concepts in Lightweight Vision-Language Models for Automated Driving](https://arxiv.org/abs/2603.06054)
*Nikos Theodoridis,Reenu Mohandas,Ganesh Sistu,Anthony Scanlan,Ciarán Eising,Tim Brophy*

Main category: cs.CV

TL;DR: 该研究通过分析视觉语言模型（VLMs）的中间激活，探究其在自动驾驶相关简单视觉任务上的失败原因，识别出感知失败和认知失败两种故障模式，并发现物体距离增加会降低视觉概念的线性可分性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在自动驾驶应用中日益普及，旨在利用其推理和泛化能力处理长尾场景，但这些模型经常在自动驾驶高度相关的简单视觉问题上失败，且失败原因尚不明确。研究旨在通过分析模型中间激活来理解这些失败的根本原因。

Method: 创建仅针对特定视觉概念存在差异的反事实图像集，然后使用四种最先进视觉语言模型的激活训练线性探针来区分这些图像。通过分析中间激活，评估特定视觉概念是否被线性编码，从而识别视觉信息流中的瓶颈。

Result: 结果显示：物体或智能体存在等概念被明确线性编码，而物体方向等空间视觉概念仅通过视觉编码器保留的空间结构隐式编码。研究发现两种故障模式：感知失败（所需视觉信息未线性编码）和认知失败（视觉信息存在但模型未能正确与语言语义对齐）。此外，物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究通过分析视觉语言模型的中间激活，提高了对自动驾驶相关简单视觉任务失败案例的理解，识别了感知和认知两种故障模式，并揭示了物体距离对视觉概念编码的影响，为改进视觉语言模型在自动驾驶应用中的性能提供了重要见解。
...
TL;DR: TempoSyncDiff：基于参考条件的潜在扩散框架，通过师生蒸馏实现高效音频驱动说话头生成，减少推理步骤并提高稳定性
...
Method: 采用参考条件的潜在扩散框架，通过师生蒸馏方法：训练标准噪声预测目标的扩散教师模型，指导轻量级学生去噪器以显著减少推理步骤；结合身份锚定和时间正则化减少身份漂移和闪烁；使用基于视位的音频条件提供粗粒度唇部运动控制

Result: 在LRS3数据集上评估，蒸馏扩散模型能保留强教师模型的大部分重建特性，同时实现显著降低的延迟推理；包括CPU和边缘计算的延迟测量，展示了边缘部署的可行性

Conclusion: 蒸馏扩散模型在保持重建质量的同时能大幅降低推理延迟，该研究是迈向受限计算环境下实用扩散基说话头生成的初步步骤
...
TL;DR: 提出基于自适应径向投影的傅里叶幅度谱文档倾斜估计方法，并构建DISE-2021评估数据集，方法在鲁棒性和准确性上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统中的关键任务，直接影响后续处理步骤的质量。随着数字化时代的到来，需要更准确、鲁棒的倾斜估计方法。

Method: 提出一种新颖的倾斜估计方法：在2D离散傅里叶幅度谱上应用自适应径向投影来提取文档图像的主导倾斜角度。

Result: 方法表现出鲁棒性和可靠性，在所有对比方法中性能最优。同时构建了高质量的DISE-2021倾斜估计数据集用于评估。

Conclusion: 提出的基于傅里叶幅度谱的自适应径向投影方法在文档倾斜估计任务上优于现有方法，为傅里叶基方法提供了全面的改进分析。
...
TL;DR: 本文提出情感连续说话人脸生成（EC-TFG）新任务，旨在生成能反映情感变化的说话视频，并开发了TIE-TFG模型，通过时序密集情感波动建模实现平滑情感过渡。


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法通常只能表达固定目标情感，缺乏像人类一样连续变化的自然表情表达能力，无法真实反映情感动态变化。

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，通过分析输入文本和情感描述生成情感变化序列，驱动合成视频中的连续面部表情变化。

Result: 广泛评估表明，该方法能够产生平滑的情感过渡，并在不同情感状态下保持高质量的视觉效果和运动真实性。

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了传统TFG方法中情感表达固定化的问题，实现了更自然、连续的情感表达，推动了说话人脸生成技术的发展。
...
### [46] [LucidNFT: LR-Anchored Multi-Reward Preference Optimization for Generative Real-World Super-Resolution](https://arxiv.org/abs/2603.05947)
*Song Fei,Tian Ye,Sixiang Chen,Zhaohu Xing,Jianyu Lai,Lei Zhu*

Main category: cs.CV

TL;DR: LucidNFT提出了一种用于真实世界图像超分辨率的强化学习框架，通过引入LR参考的忠实度评估器、解耦的优势归一化策略和大规模真实退化数据集，解决了现有方法中的语义幻觉、优势崩溃和退化覆盖不足等问题。


<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率存在关键问题：输出可能看起来清晰但不符合LR证据（语义和结构幻觉），而LR锚定的忠实度难以评估。基于偏好的强化学习虽然适合，但受到三个主要限制：缺乏退化鲁棒的LR参考忠实度信号、rollout组优化瓶颈导致优势崩溃，以及真实退化覆盖有限限制了rollout多样性和偏好信号质量。

Method: LucidNFT是一个用于流匹配真实世界图像超分辨率的多奖励强化学习框架。包含三个核心组件：1) LucidConsistency - 退化鲁棒的语义评估器，使LR锚定的忠实度可测量和可优化；2) 解耦的优势归一化策略 - 在融合前保留每个LR条件rollout组内的目标间对比，防止优势崩溃；3) LucidLR - 大规模真实世界退化图像集合，支持鲁棒的RL微调。

Result: 实验表明，LucidNFT持续改进了基于流的真实世界图像超分辨率基线，在多样真实世界场景中实现了更好的感知-忠实度权衡，并具有稳定的优化动态。

Conclusion: LucidNFT通过解决退化鲁棒忠实度评估、优势崩溃和退化覆盖不足等关键问题，为真实世界图像超分辨率提供了一个有效的强化学习框架，能够生成既视觉上令人信服又忠实于LR证据的高质量超分辨率结果。
...
### [47] [Unify the Views: View-Consistent Prototype Learning for Few-Shot Segmentation](https://arxiv.org/abs/2603.05952)
*Hongli Liu,Yu Wang,Shengjie Zhao*

Main category: cs.CV

TL;DR: VINE提出统一框架解决少样本分割中的结构错位和跨视角不一致问题，通过空间-视角图建模结构一致性，利用支持-查询特征差异获取判别性先验，结合SAM和ResNet特征生成类别一致的原型作为自适应提示。


<details>
  <summary>Details</summary>
Motivation: 少样本分割在大外观或视角变化下存在结构错位和跨视角不一致的挑战，需要同时建模结构一致性和前景判别性来改进类别特定原型。

Method: 1) 在骨干特征上构建空间-视角图：空间图捕捉局部几何拓扑，视角图连接不同视角特征传播视角不变的结构语义；2) 从支持-查询特征差异推导判别性先验，通过强调显著区域重新加权SAM特征并重新校准骨干激活；3) 通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致原型作为SAM解码器的自适应提示。

Result: 在多个少样本分割基准测试上的广泛实验验证了VINE的有效性和鲁棒性，特别是在具有视角变化和复杂结构的挑战性场景中表现优异。

Conclusion: VINE通过联合建模结构一致性和前景判别性，有效解决了少样本分割中的结构错位和跨视角不一致问题，为视角变化下的分割任务提供了鲁棒解决方案。
...
TL;DR: OVGGT是一个训练无关的框架，通过自选择性缓存和动态锚点保护技术，在固定内存和计算预算下处理任意长度视频的3D几何重建，解决了现有方法在长序列中的内存爆炸问题。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全对全注意力机制，其二次方计算成本限制了只能处理短序列；因果注意力变体如StreamVGGT虽然支持单次流式处理，但会积累不断增长的KV缓存，在数百帧内耗尽GPU内存，无法实现真正意义上的长序列流式推理。

Method: OVGGT结合两种核心技术：1) 自选择性缓存，利用FFN残差幅度来压缩KV缓存，同时保持与FlashAttention的完全兼容；2) 动态锚点保护，保护坐标关键令牌不被驱逐，以抑制在扩展轨迹上的几何漂移。

Result: 在室内、室外和超长序列基准测试上的广泛实验表明，OVGGT能够在固定的VRAM限制内处理任意长度的视频，同时实现最先进的3D几何精度。

Conclusion: OVGGT提供了一个训练无关的解决方案，将内存和计算都限制在固定预算内，无论序列长度如何，实现了真正可扩展的长序列3D几何重建，为流式视频推理的实际部署铺平了道路。
...
TL;DR: Place-it-R1是一个端到端的视频对象插入框架，利用多模态大语言模型的链式思维推理能力，通过"思考-放置"范式实现物理一致的视频编辑，提供可调节的物理合理性与视觉保真度权衡。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的对象与环境物理不一致。需要一种能够理解物理场景并进行环境感知推理的方法，以实现物理一致的视频对象插入。

Method: 采用"思考-放置"范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的链式思维令牌并推断有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过MLLM评分优化视觉自然度；3) 推理时MLLM迭代触发细化循环，形成闭环渐进增强编辑质量；4) 提供两种用户可选模式：允许环境修改的合理性导向灵活模式，以及保持场景完整性的保真度导向标准模式。

Result: 大量实验表明，Place-it-R1相比最先进解决方案和商业模型，能够实现物理一致的视频对象插入，在物理连贯性方面表现优异。

Conclusion: 该工作展示了利用MLLM的认知推理能力与扩散模型的生成能力相结合，通过闭环迭代优化，能够实现物理一致且视觉自然的视频对象插入，为用户提供了物理合理性与视觉保真度的明确权衡控制。
...
TL;DR: 本文提出了一种基于两阶段策略（目标分割+识别）的开放词汇目标识别框架，通过CLIP生成图像和文本嵌入，并引入CNN/MLP方法增强编码灵活性，无需复杂重训练和标注
...
Method: 采用两阶段策略：先进行目标分割裁剪区域，然后使用CLIP生成目标级图像嵌入和类别级文本嵌入；引入CNN/MLP方法提取CNN特征图并用MLP对齐视觉特征与文本嵌入；通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练的CLIP编码（无SVD）取得了最高的平均AP，超越了当前最先进方法；CNN/MLP图像编码在OVOR中显示出潜力

Conclusion: 提出的两阶段OVOR框架有效解决了现有方法的局限性，无需复杂重训练和标注，CLIP编码表现最佳，CNN/MLP方法为开放词汇识别提供了新的编码可能性
...
TL;DR: VLM-RobustBench是一个评估视觉语言模型在真实世界图像失真下鲁棒性的基准测试，涵盖49种增强类型，133种损坏设置。研究发现当前VLMs语义理解强但空间鲁棒性脆弱，几何和重采样失真对性能影响最大。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现良好，但对其在真实世界图像失真下的性能缺乏全面理解。需要系统评估VLMs对各种图像扰动的鲁棒性，以揭示其实际应用中的局限性。

Method: 提出VLM-RobustBench基准测试，涵盖49种增强类型（噪声、模糊、天气、数字、几何扰动），在分级严重程度（低/中/高）和二元变换下评估，共133种损坏设置。评估四个VLM家族（Qwen、InternVL、Molmo、Gemma）在两个互补基准上：MMBench（视觉基础）和MMMU-Pro（推理导向）。

Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动比视觉上严重的光度损坏影响更大。低严重度glass_blur使MMBench准确率平均下降约8个百分点，最大下降来自重采样和几何失真（如upsample、elastic_transform），可达34个百分点。当前VLMs语义理解强但空间鲁棒性脆弱。

Conclusion: 研究结果表明需要定义新的鲁棒性评估协议和训练机制，强调重采样和几何不变性。当前VLMs在空间扰动下表现脆弱，这为未来模型设计和训练提供了重要方向。
...
TL;DR: 提出RF-Sampling，一种针对流匹配模型（特别是CFG蒸馏变体如FLUX）的理论基础训练免费推理增强框架，通过线性组合文本表示和流反演来提升生成质量和提示对齐


<details>
  <summary>Details</summary>
Motivation: 现有推理增强策略主要适用于传统扩散模型，在流模型上表现不佳，需要专门为流模型设计的推理增强方法

Method: RF-Sampling通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，利用文本表示的线性组合并结合流反演，使模型探索与输入提示更一致的噪声空间

Result: 在多个基准测试中，RF-Sampling一致地提升了生成质量和提示对齐，是首个在FLUX上展现一定程度测试时缩放能力的推理增强方法

Conclusion: RF-Sampling为流模型提供了有效的训练免费推理增强框架，填补了现有技术在该类模型上的空白，具有理论支撑和实际效果
...
TL;DR: CR-QAT框架通过课程量化感知训练和文本中心关系知识蒸馏，解决了开放词汇目标检测模型在极端低比特量化下视觉-语言对齐和区域关系结构退化的问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测模型尺寸庞大，难以在资源受限设备上部署。虽然量化提供实用压缩方案，但极端低比特（如4位）量化会严重破坏细粒度视觉-语言对齐并扭曲区域间关系结构。

Method: 提出课程关系量化感知训练（CR-QAT）框架，包含两个核心组件：1）课程QAT（CQAT）通过分区渐进量化缓解误差累积，确保稳定优化；2）文本中心关系知识蒸馏（TRKD）通过构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下持续优于现有QAT基线，相对AP提升分别达到38.9%和40.9%。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测模型在极端量化下的性能退化问题，通过课程化渐进量化和关系知识蒸馏的结合，实现了高效模型压缩同时保持视觉-语言对齐能力。
...
TL;DR: 提出两阶段框架解决字形相似度度量问题：第一阶段在标注的人工字母表上训练对比学习编码器作为教师模型，第二阶段通过师生蒸馏扩展到历史文字系统，学生模型在教师指导下学习无监督表示，发现跨文字系统的潜在相似性。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统的相似度度量面临根本挑战：人工字母表中的单个字素可以可靠标注，但不同文字系统之间的历史关系不确定且有争议。需要一种方法既能利用标注数据建立可靠特征，又能扩展到历史文字系统而不依赖有争议的进化关系标注。

Method: 两阶段框架：1）在标注的人工字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2）通过师生蒸馏扩展到历史文字系统，学生模型学习无监督表示，在教师知识指导下但能自由发现跨文字系统的潜在相似性。非对称设置使学生能学习变形不变的嵌入，同时继承干净示例的判别结构。

Result: 在多样化的文字系统上实验表明，该方法能有效进行少样本字形识别，并在不需要真实进化关系标注的情况下实现有意义的文字聚类。框架成功桥接了监督对比学习和无监督发现，既能区分不同文字系统的硬边界，又能反映潜在历史影响的软相似性。

Conclusion: 提出的两阶段框架解决了字形相似度度量的认识论约束，通过师生蒸馏将监督学习的判别能力扩展到历史文字系统，实现了既有判别性又有发现性的字形表示学习，为文字系统研究提供了有效的计算工具。
...
TL;DR: DynUAV是一个新的动态无人机视角多目标跟踪基准，包含剧烈自运动和复杂表观轨迹，旨在解决现有基准缺乏真实动态复杂性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角多目标跟踪基准通常缺乏真实世界的动态复杂性，主要包含可预测的相机运动和线性运动模式，无法反映无人机快速移动和敏捷机动带来的显著观测挑战。

Method: 提出了DynUAV基准，包含42个视频序列和超过170万个边界框标注，涵盖车辆、行人以及挖掘机、推土机、起重机等专业工业类别。该基准以剧烈自运动和由此产生的复杂表观轨迹为特征。

Result: 与现有基准相比，DynUAV引入了由自运动带来的重大挑战，包括剧烈的尺度变化、视角变化和运动模糊。对最先进跟踪器的综合评估揭示了它们在处理这种动态条件下的检测和关联交织挑战时的局限性。

Conclusion: DynUAV建立了一个严格的基准，预计将作为苛刻的测试平台，推动真实世界无人机视角多目标跟踪的进展，所有资源将公开提供。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM在图像描述和视觉问答等任务上表现优异，但在准确解释视觉数据中的深度信息方面存在困难。深度信息对于三维场景理解至关重要，但传统方法主要关注RGB图像与文本的对齐，未能充分利用深度图像的特性。

Method: 1. 构建新颖的深度图像-文本对数据集和深度指令数据集：使用GLPN模型从视觉图像生成深度图，利用GPT-4生成对应的深度指令，并通过LLaVA验证。2. 改进CLIP的ViT编码器：融入局部物体信息，以更有效地捕捉深度图的细微连续变化。3. 开发全面的深度问答基准：基于现有深度图像数据集，用于评估模型在典型深度图场景中的理解能力。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门的深度MLLM，通过充分利用深度图像的单通道灰度特性（像素值直接反映深度线索），有效解决了传统方法在深度信息理解方面的局限性，为三维场景理解提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：1) 依赖领域特定的监督；2) 隐式融合导致跨模态对齐弱；3) 平坦的时间建模缺乏事件过渡。需要一种无需训练、具有显式跨模态对齐和结构化事件推理的框架。

Method: 提出CoE框架，基于层次化事件图(HEG)进行结构化推理。HEG将文本语义编码为显式事件层次结构，指导跨模态对齐和时间推理。CoE通过事件链定位关键视觉线索，建模事件演化和因果过渡，并通过轻量级风格适应进行领域对齐。

Result: 在八个多样化数据集上的实验表明，CoE持续优于最先进的视频CoT基线，平均提升+3.04 ROUGE、+9.51 CIDEr和+1.88 BERTScore，展现了其鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过结构化的事件链推理和层次化事件图，有效解决了多模态摘要中的跨模态对齐、事件建模和领域泛化问题，提供了一种无需训练、可解释且泛化能力强的解决方案。
...
TL;DR: TaPD是一个用于可变历史长度轨迹预测的统一即插即用框架，通过渐进知识蒸馏和时序回填模块解决短观测序列的性能下降问题


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在实际场景中（如遮挡或感知范围限制）观测长度可变或极短，导致性能显著下降。需要一种能够适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协作模块：1) 观测自适应预测器(OAF)，基于渐进知识蒸馏(PKD)将长时程"教师"的运动模式知识通过分层特征回归转移到短时程"学生"；2) 时序回填模块(TBM)，为极短历史显式重建缺失的历史片段。采用余弦退火蒸馏权重方案平衡预测监督和特征对齐，并使用解耦的预训练-重建-微调协议。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下都一致优于强基线方法，在极短输入下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD通过渐进知识蒸馏和时序回填的协同作用，有效解决了可变历史长度轨迹预测问题，为实际自动驾驶场景中的观测不确定性提供了鲁棒的解决方案。
...
TL;DR: RePer-360是一个针对单目全景深度估计的失真感知自调制框架，通过轻量级几何对齐引导模块和自条件AdaLN-Zero机制，在保持预训练透视先验的同时适应全景域，仅需1%训练数据即可超越标准微调方法


<details>
  <summary>Details</summary>
Motivation: 现有的深度基础模型在透视图像上训练表现良好，但在360°全景图像上泛化能力差，主要原因是透视域与全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，而获取这些数据成本高昂
...
Result: RePer-360仅使用1%的训练数据就超越了标准微调方法。在相同域内训练设置下，RMSE指标实现了约20%的改进。该方法通过从互补投影融合转向在保持预训练透视先验下的全景域适应，取得了显著性能提升

Conclusion: RePer-360成功解决了深度基础模型在全景图像上的泛化问题，通过失真感知自调制框架实现了高效的全景域适应，在保持预训练透视先验的同时显著提升了全景深度估计性能，为跨域深度估计提供了有效的解决方案
...
TL;DR: GazeMoE：基于混合专家机制，从冻结的基础模型中自适应选择多模态线索进行视线目标估计的端到端框架


<details>
  <summary>Details</summary>
Motivation: 从可见图像中估计人类视线目标是机器人理解人类注意力的关键任务，但现有方法在泛化性神经架构和训练范式方面仍面临挑战。虽然预训练视觉基础模型为定位视线目标提供了有前景的途径，但如何有效整合眼睛、头部姿态、手势和上下文特征等多模态线索需要自适应且高效的解码机制。

Method: 提出GazeMoE框架，受大型视觉语言模型中混合专家（MoE）机制的启发，通过MoE模块从冻结的基础模型中自适应选择与视线目标相关的线索。为解决视线目标分类（框内vs框外）中的类别不平衡问题并增强鲁棒性，引入了类别平衡辅助损失和策略性数据增强，包括区域特定裁剪和光度变换。

Result: 在基准数据集上的广泛实验表明，GazeMoE在具有挑战性的视线估计任务上实现了最先进的性能，超越了现有方法。

Conclusion: GazeMoE通过混合专家机制有效整合多模态线索，解决了视线目标估计中的泛化性和鲁棒性问题，为机器人理解人类注意力提供了有效的解决方案。
...
Method: 采用统一推理-生成框架：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 构建语义-视觉双路径引导机制；4) 创建包含130k视频、3k特效类别的EffectData数据集。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了可扩展且灵活的自定义特效生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了现有AIGC系统在视觉特效生成中的关键挑战，实现了无需逐特效微调的准确、可控、特效一致的合成，为定制化VFX生成提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛使用，理解其决策过程变得至关重要。传统的可解释性方法设计用于分类任务，难以处理现代自回归VLM，因为其复杂的逐token生成过程以及视觉和文本模态之间错综复杂的交互。

Method: DEX-AR通过在逐token生成过程中计算相对于注意力图的层梯度来解释自回归VLM，包括不同层和生成token的重要性变化。该方法引入两个关键创新：1）动态头过滤机制，识别专注于视觉信息的注意力头；2）序列级过滤方法，聚合每token解释同时区分视觉基础和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，在使用新的归一化困惑度度量的基于扰动的指标以及基于分割的指标上都有一致的改进。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够处理其复杂的生成过程和跨模态交互，在多个基准测试中表现出优于传统方法的性能。
...
TL;DR: MOSIV：首个针对视频中多物体系统识别的框架，通过可微分模拟器直接优化连续物体材料参数，在接触丰富的多物体交互场景中显著提升识别精度和长期模拟保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单物体场景或基于固定材料原型的离散分类，无法处理视频中多物体系统识别这一挑战性问题。需要开发能够直接优化连续物体材料参数的新方法。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、每个物体的材料参数，通过从视频中提取的几何目标进行指导。创建了包含接触丰富的多物体交互的合成基准数据集。

Result: 在新建的基准测试中，MOSIV相比适应后的基线方法，显著提高了接地准确性和长期模拟保真度，成为该新任务的强基线。分析表明物体级细粒度监督和几何对齐目标对复杂多物体场景的稳定优化至关重要。

Conclusion: MOSIV为视频中的多物体系统识别提供了有效解决方案，证明了物体级细粒度监督和几何对齐目标的重要性。代码和数据集将开源，为后续研究奠定基础。
...
TL;DR: K-MaT是一个知识锚定的流形传输框架，通过提示学习将决策结构从高端医学影像模态（如CT）迁移到低端模态（如X光），无需低端训练图像，使用融合Gromov-Wasserstein最优传输对齐提示流形。


<details>
  <summary>Details</summary>
Motivation: 大规模生物医学视觉语言模型（VLMs）在高端影像模态（如CT）上适配后，往往无法有效迁移到前线低端模态（如X光），容易陷入模态特定的捷径学习。现有方法在跨模态部署时面临灾难性遗忘和性能下降的问题。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）实现零样本跨模态迁移，无需低端训练图像。

Result: 在四个跨模态基准测试（皮肤镜检查、乳腺X光到超声、CT到胸部X光）上取得最先进结果：平均调和准确率从BiomedCoOp的42.0%提升到44.1%，宏F1分数达到36.2%。在具有挑战性的乳腺成像任务中，有效缓解了标准方法（如CoOp）的灾难性遗忘问题（CoOp在低端模态准确率降至27.0%）。

Conclusion: 通过最优传输对齐提示流形为医学VLMs的零样本跨模态部署提供了一条高效路径，能够保持跨模态的稳健性能，避免灾难性遗忘。
...
### [65] [StruVis: Enhancing Reasoning-based Text-to-Image Generation via Thinking with Structured Vision](https://arxiv.org/abs/2603.06032)
*Yuanhuiyi Lyu,Kaiyu Lei,Ziqiao Weng,Xu Zheng,Lutao Jiang,Teng Li,Yangfu Li,Ziyuan Huang,Linfeng Zhang,Xuming Hu*

Main category: cs.CV

TL;DR: StruVis是一个通过结构化视觉表示增强文本到图像生成推理的新框架，避免中间图像生成的计算成本，同时保持视觉感知能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成的推理框架存在两个主要问题：纯文本推理缺乏视觉上下文导致忽略关键空间和视觉元素；文本-图像交错推理虽然增强视觉基础但计算成本高且受限于生成器的表示能力。需要一种既能感知视觉结构又计算高效的推理方法。

Method: 提出StruVis框架，使用基于文本的结构化视觉表示作为中间推理状态，让多模态大语言模型在纯文本推理过程中有效"感知"视觉结构。通过结构化视觉引导的推理解锁模型的推理潜力，该框架与生成器无关，可无缝集成到各种文本到图像生成器中。

Result: 在推理型文本到图像生成基准测试中取得显著性能提升：T2I-ReasonBench上提升4.61%，WISE基准上提升4%。证明该框架能有效增强生成器的推理能力。

Conclusion: StruVis通过结构化视觉表示实现了高效且视觉感知的推理，避免了现有方法的局限性，为文本到图像生成的推理任务提供了一种计算高效且性能优越的解决方案。
...
TL;DR: OA-SORT是一个即插即用、无需训练的遮挡感知多目标跟踪框架，通过OAM、OAO和BAM三个模块解决2D MOT中因部分遮挡导致的代价混淆问题，在多个数据集上显著提升跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪面临因部分遮挡导致的定位代价混淆问题，现有方法在处理遮挡时存在局限性，需要更有效的遮挡感知机制来提升跟踪精度和稳定性。

Method: 提出OA-SORT框架，包含三个核心模块：1) 遮挡感知模块(OAM)分析物体遮挡状态，引入高斯图减少背景影响；2) 遮挡感知偏移(OAO)利用OAM描述的遮挡状态缓解代价混淆；3) 偏置感知动量(BAM)抑制估计不稳定性。该框架即插即用且无需训练。

Result: 在DanceTrack测试集上，OA-SORT达到63.1% HOTA和64.2% IDF1。将遮挡感知框架集成到其他四个跟踪器后，HOTA和IDF1平均提升2.08%和3.05%，证明了框架的可重用性。在SportsMOT和MOT17数据集上的综合评估也验证了遮挡处理的重要性。

Conclusion: 遮挡处理是多目标跟踪中的关键问题，OA-SORT通过创新的遮挡感知机制有效解决了代价混淆问题，其即插即用特性使其能够轻松集成到现有跟踪器中，显著提升跟踪性能。
...
### [67] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
...
<details>
  <summary>Details</summary>
Motivation: SAM3等基础模型在医学图像分割中虽然支持灵活的文本引导，但其预测对提示表述高度敏感，即使是语义等价的描述也可能产生不一致的分割结果，这限制了在临床和病理工作流程中的可靠性。

Method: 将提示敏感性重新定义为组间一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出提示组感知训练框架，包括：(1) 利用分割损失作为隐式排名信号的质量引导组正则化；(2) 采用停止梯度策略的logit级一致性约束来对齐组内预测。该方法无需修改模型架构，推理过程保持不变。

Result: 在多数据集细胞核基准测试中，该方法在文本提示下获得一致性能提升，显著降低了不同提示质量水平下的性能方差。在六个零样本跨数据集任务中，平均Dice系数提高了2.16个百分点。

Conclusion: 该方法显著提升了视觉-语言分割在计算病理学中的鲁棒性和泛化能力，为解决基础模型在医学图像分割中的提示敏感性问题提供了有效方案。
...
TL;DR: 该研究针对高维超列特征在图像分割中的计算复杂度问题，提出分层子采样和集成学习方法，在脑肿瘤数据集上显著提升了小样本情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列特征虽然能有效用于图像分割，但由于其计算复杂度随训练集大小线性增长，实际应用受限。本研究旨在解决超列特征处理的计算效率问题，特别是在小样本情况下的性能优化。

Method: 采用基于VGG16的超列特征，应用分层子采样策略降低计算复杂度，并研究集成学习方法（包括堆叠和投票集成）在稀疏超列上的性能表现。

Result: 在脑肿瘤数据集上，当训练样本N≤20时，简单的逻辑回归分类器表现最佳。在10%分层子采样率下，N=20时最佳平均Dice得分为0.66，相比标准多尺度UNet基线有24.53%的显著提升（p值=3.07e-11）。

Conclusion: 分层子采样和集成学习方法能有效解决超列特征的计算复杂度问题，在小样本情况下显著提升图像分割性能，特别是在极端低样本情况下，简单分类器比复杂模型更有效。
...
### [69] [Physical Simulator In-the-Loop Video Generation](https://arxiv.org/abs/2603.06408)
*Lin Geng Foo,Mark He Huang,Alexandros Lattas,Stylianos Moschoglou,Thabo Beeler,Christian Theobalt*

Main category: cs.CV

TL;DR: PSIVG框架通过将物理模拟器集成到视频扩散过程中，解决了AI生成视频中物体运动违反物理定律的问题，生成更符合现实物理规律的视频。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成方法虽然在视觉真实感方面取得了显著进展，但生成的物体运动经常违反重力、惯性和碰撞等基本物理定律，导致运动不一致、动力学不真实或违反物理约束，限制了AI生成视频的真实感和可靠性。

Method: 提出物理模拟器在环视频生成（PSIVG）框架：1）从预训练扩散模型生成的模板视频重建4D场景和前景物体网格；2）在物理模拟器中初始化这些网格并生成物理一致的轨迹；3）用模拟轨迹指导视频生成器产生时空物理一致的运动；4）提出测试时纹理一致性优化（TTCO）技术，基于模拟器的像素对应关系调整文本和特征嵌入以改善纹理一致性。

Result: 综合实验表明，PSIVG生成的视频能更好地遵循现实世界物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过将物理模拟器集成到视频生成流程中，有效解决了当前AI生成视频违反物理定律的问题，提高了生成视频的物理真实性和可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成高质量图像方面表现出色，但在排版控制方面存在局限：模型经常忽略或弱化用户对字体样式的要求。需要解决这一挑战，使模型能更好地理解和执行排版相关的提示。

Method: 采用数据为中心的方法，构建大规模排版数据集FontUse（约70K图像），使用专门设计的结构化标注流程自动生成标注。流程结合分割模型和多模态大语言模型，自动标注用户友好提示、文本区域位置和OCR识别字符串。提示明确结合字体样式（如衬线体、手写体、优雅字体）和使用场景（如婚礼请柬、咖啡店菜单）。基于此数据集微调现有生成模型，无需修改架构。

Result: 实验表明，使用该流程训练的模型在多样化的提示和布局下，生成的文本渲染与提示要求的一致性优于竞争基线。论文还引入了基于Long-CLIP的评估指标来测量生成排版与请求属性之间的对齐度。

Conclusion: 通过数据驱动的标注流程构建FontUse数据集，能够有效提升文本到图像模型在排版控制方面的能力，使模型能更一致地解释样式和使用场景条件，为排版生成提供了实用解决方案。
...
TL;DR: CLoPA是一种持续适应策略，通过轻量级训练调整nnInteractive模型的小部分参数，在医学图像分割任务中快速达到专家级性能


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型如nnInteractive在不同医学成像任务中无法稳定达到专家级性能，而标注活动产生的任务特定标注数据流为在线模型适应提供了自然补充

Method: 提出CLoPA持续适应策略，在标注缓存上调整nnInteractive模型的小部分参数，通过轻量级训练调度触发，无需新参数或改变推理流程

Result: 在8个医学分割十项全能任务中，CLoPA快速将性能提升至专家级，即使在nnInteractive先前失败的任务中，多数增益在单个训练周期后实现

Conclusion: 不同参数组调优的效益取决于任务特性和数据机制；对于复杂几何目标，实例归一化和低级特征调优会饱和，表明在最挑战场景中需要更深层特征表示对齐
...
<details>
  <summary>Details</summary>
Motivation: 统一多模态模型在视觉理解方面表现出色，但在生成能力方面相对较弱，存在理解与生成过程的内在解耦问题。模型能够准确理解细粒度视觉细节，却难以从复杂文本提示生成语义一致的图像。

Method: 提出GvU方法：1) 基于理解分支的token级内在文本-图像对齐奖励机制，使模型同时作为教师和学生；2) 自监督强化学习框架，通过理解分支的内在奖励信号迭代提升生成质量，无需外部监督。
...
<details>
  <summary>Details</summary>
Motivation: 当前手-物体交互视频合成面临两大挑战：1）专用HOI重演方法在复杂野外场景泛化能力差；2）通用视频编辑模型在处理HOI特定问题（如物体外观不一致）时表现不佳。需要一种既能保持背景真实感又能增强交互保真度的解决方案。

Method: 提出GenHOI框架，包含两个核心创新：1）时间平衡机制：采用Head-Sliding RoPE，为参考token分配头部特定的时间偏移，均匀分布其跨帧影响力，缓解3D RoPE的时间衰减问题；2）空间选择性机制：设计两级空间注意力门，将物体条件注意力集中在HOI区域，并自适应调整强度。

Result: 在未见过的野外场景上进行广泛定性和定量评估，GenHOI在物体一致性、交互物理合理性和背景保真度方面显著优于最先进的HOI重演方法和通用视频编辑方法。

Conclusion: GenHOI通过轻量级增强策略有效解决了HOI视频合成的关键挑战，在保持预训练模型鲁棒性的同时，显著提升了手-物体交互的生成质量，为复杂野外场景的HOI视频合成提供了实用解决方案。
...
TL;DR: Curious-VLA框架通过两阶段设计解决VLA模型的窄策略限制：IL阶段使用可行轨迹扩展和归一化轨迹表示来增加数据多样性；RL阶段采用自适应多样性感知采样和跨度驾驶奖励来提升探索能力和驾驶质量敏感性，在Navsim基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 识别出自主VLA模型存在窄策略限制问题：模仿学习倾向于坍缩探索，限制了后续强化学习阶段的潜力，导致其因反馈多样性不足而过早饱和。

Method: 提出Curious-VLA两阶段框架：1) IL阶段：引入可行轨迹扩展策略生成多个物理有效轨迹，并使用步进归一化轨迹表示来适应这种多样性数据；2) RL阶段：提出自适应多样性感知采样优先选择高多样性样本，并引入跨度驾驶奖励和焦点风格加权来放大奖励值跨度，提升对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得了SOTA结果：PDMS 90.3，EPDMS 85.4，以及Best-of-N PDMS 94.8，证明了其在释放VLA模型探索潜力方面的有效性。

Conclusion: Curious-VLA通过解决模仿学习中的探索坍缩问题和强化学习中的反馈多样性不足，有效缓解了利用-探索困境，显著提升了VLA模型的性能和探索能力。
...
TL;DR: 基于45,139张超声图像训练的AI系统，在胎儿口面裂诊断中达到93%敏感性和95%特异性，与资深放射科医生相当，显著优于初级医生，并能提升初级医生6%以上的敏感性，同时加速临床专业知识培养。


<details>
  <summary>Details</summary>
Motivation: 口面裂是最常见的先天性颅面异常之一，但产前准确检测面临挑战：经验丰富的专家稀缺，且该疾病相对罕见。早期可靠诊断对于及时临床干预和降低相关发病率至关重要。

Method: 开发了一个人工智能系统，基于来自22家医院、9,215名胎儿的45,139张超声图像进行训练。该系统可作为医学副驾驶辅助诊断，并通过包含24名放射科医生和学员的试点研究评估其对专业知识发展的促进作用。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，并显著优于初级放射科医生。作为医学副驾驶使用时，能将初级放射科医生的敏感性提升超过6%。试点研究表明该系统能加速罕见疾病的专业知识发展。

Conclusion: 这种双重用途方法为改善诊断准确性和专家培训提供了可扩展的解决方案，特别适用于经验丰富的放射科医生稀缺的环境，既能直接辅助诊断，又能加速临床专业知识发展。
...
TL;DR: SUREON是一个大规模外科视频问答数据集，通过从外科教学视频中提取专家讲解来训练AI理解外科推理，包括意图、风险评估和预测等复杂问题。


<details>
  <summary>Details</summary>
Motivation: 当前外科AI缺乏理解外科推理的能力（如为什么选择特定器械、风险评估、下一步操作等），因为大规模标注这类数据极其困难。然而，外科教学视频中专家讲解已经包含了这些推理信息，只是未被结构化利用。

Method: 1) 创建SUREON数据集：从外科学术视频中提取讲解，定义12个问题类别（安全评估、决策理由、预测等），使用多智能体管道大规模提取和结构化监督数据；2) 开发两个模型：SureonVLM（通过监督微调适应的视觉语言模型）和SureonVLM-R1（使用Group Relative Policy Optimization训练的逻辑推理模型）。

Result: SUREON包含134.7K个视频片段、170种手术类型、206.8k个QA对，以及354个专家验证的基准测试。两个模型在复杂外科问题上表现优异，SureonVLM-R1在SUREON基准上准确率超过84%，优于更大的通用领域模型，并在标准外科感知任务上表现更好。定性分析显示模型能根据视觉上下文推断手术意图。

Conclusion: 外科教学视频中的专家讲解是训练AI理解外科推理的宝贵资源。SUREON数据集和相应模型证明，通过结构化利用这些"噪声"讲解，可以显著提升AI的外科推理能力，为开发更智能的外科AI系统奠定了基础。
...
TL;DR: BEVLM框架将空间一致的BEV表示与LLM连接，通过BEV特征作为统一输入提升跨视图驾驶场景推理能力，同时将LLM语义知识蒸馏到BEV表示中改善端到端驾驶性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLM导致计算冗余和空间一致性有限，阻碍准确的3D空间推理和几何一致性。而传统BEV表示虽有空间结构但缺乏基础视觉编码器的语义丰富性。

Method: 提出BEVLM框架，连接空间一致且语义蒸馏的BEV表示与LLM，使用BEV特征作为统一输入，并将LLM的语义知识蒸馏到BEV表示中。

Result: BEVLM使LLM在跨视图驾驶场景中推理更有效，准确率提升46%；通过将语义知识蒸馏到BEV表示中，在安全关键场景下闭环端到端驾驶性能提升29%。

Conclusion: BEVLM成功桥接了BEV的空间一致性与LLM的语义理解能力，显著提升了自动驾驶系统的推理准确性和端到端驾驶性能。
...
TL;DR: FedARKS框架通过鲁棒知识提取和知识选择机制，解决联邦域泛化行人重识别中全局特征不足和参数平均化的问题


<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征表示和简单参数平均，存在两个局限：1) 仅用全局特征难以捕捉细粒度域不变局部细节；2) 统一参数平均忽视客户端在鲁棒特征提取能力上的差异，稀释高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制提取细粒度局部特征以增强域不变性；KS（知识选择）机制根据客户端特征提取能力差异进行选择性聚合，避免高质量知识被稀释

Result: 论文未提供具体实验结果，但方法旨在提升模型在未见域上的泛化能力，同时保护客户端数据隐私

Conclusion: FedARKS通过结合细粒度局部特征提取和差异化知识选择，有效解决了联邦域泛化行人重识别中的关键挑战，为隐私保护下的跨域泛化提供了新思路
...
TL;DR: RMD提出跨分辨率分布匹配蒸馏框架，通过logSNR曲线划分时间步、分布匹配和噪声重注入机制，在保持高保真度的同时实现33.4倍加速


<details>
  <summary>Details</summary>
Motivation: 现有扩散蒸馏方法受限于去噪过程，时间步减少已基本饱和；部分时间步低分辨率生成虽能加速但存在跨分辨率分布差距导致质量下降

Method: 1) 使用logSNR曲线为每个分辨率划分时间步区间；2) 引入logSNR映射补偿分辨率引起的偏移；3) 沿分辨率轨迹进行分布匹配；4) 在上采样时加入预测噪声重注入机制

Result: RMD在保持高视觉保真度的同时显著加速推理：SDXL上达33.4倍加速，Wan2.1-14B上达25.6倍加速，定量和定性结果均显示高质量生成

Conclusion: RMD通过跨分辨率分布匹配有效解决了低分辨率生成中的分布差距问题，实现了高保真度的少步多分辨率级联推理，为扩散模型加速提供了新方向
...
TL;DR: 该论文研究视觉语言模型在结构化色彩扭曲下的感知弱点，发现即使场景对人类仍可识别，模型预测也会严重退化，并提出人类启发的预处理方法提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但在面对结构化、大幅度的像素值变化时，即使底层场景对人类仍然容易识别，模型也会产生自信但无意义的预测。这种系统性的感知弱点需要被研究和解决。

Method: 提出空间色彩混合框架，包含八种程序化色彩扭曲变体（使用RGB和Ostwald色彩系统），在四个数据集上评估九个来自三个模型家族的视觉语言模型。同时进行包含61名参与者的人类研究，并测试人类启发的预处理方法。

Result: 随着扭曲程度增加，所有模型和数据集上的准确率急剧下降；扩大语言模型规模并不能可靠缓解这种失败；人类在相同扭曲下显著优于视觉语言模型；简单的人类启发预处理方法能够为多种扭曲类型恢复相当一部分性能。

Conclusion: 视觉语言模型存在系统性感知弱点，需要感知感知的预处理和工具使用等实用策略来提高鲁棒性。人类在结构化色彩扭曲下的优越表现表明模型仍有改进空间。
...
<details>
  <summary>Details</summary>
Motivation: 预测放疗期间肿瘤演变是临床关键挑战，特别是当纵向变化由解剖结构和治疗共同驱动时。现有方法难以准确模拟放疗诱导的解剖变化，需要开发能够整合剂量信息和临床变量的预测框架。

Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知的多模态条件图像到图像转换问题。基于CT扫描、基线临床变量和指定辐射剂量增量，合成反映治疗诱导解剖变化的随访CT图像。使用222名III期NSCLC患者的895个CT扫描数据集，在2D和2.5D配置下对基于GAN和扩散的模型进行基准测试。

Result: 定量和定性结果表明，基于扩散的模型更一致地从多模态、剂量感知的条件中受益，产生比基于GAN的基线更稳定、解剖学上更合理的肿瘤演变轨迹。扩散模型在合成放疗诱导的解剖变化方面表现更优。

Conclusion: 虚拟治疗框架作为NSCLC计算机模拟治疗监测和自适应放疗研究的工具有潜力。扩散模型在剂量感知条件图像合成方面优于GAN模型，能够生成更稳定、解剖学合理的肿瘤演变轨迹。
...
TL;DR: FreeOcc是一个无需训练的道路场景语义和全景占据预测框架，利用预训练基础模型从多视角图像恢复语义和几何信息，在未见环境中实现3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的占据预测方法通常需要昂贵的密集3D监督或针对目标域的训练数据，限制了在未见环境中的部署。需要一种无需训练的方法来克服这些限制。

Method: 1) 使用可提示的基础分割模型提取每视角全景先验，通过提示到分类规则处理语义；2) 使用重建基础模型恢复度量3D点；3) 通过深度和置信度感知过滤将可靠标签提升到3D；4) 时间融合和确定性细化堆栈进行体素化；5) 通过拟合和合并鲁棒的当前视角3D框候选恢复实例，实现无需学习3D模型的实例感知占据。

Result: 在Occ3D-nuScenes上，FreeOcc无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超越先前最先进的弱监督基线。在全景占据预测方面，分别达到3.1 RayPQ（无需训练）和3.9 RayPQ（弱监督），建立了新的基准。
...
<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在乳腺超声图像分割中，由于标注数据极度有限，生成的伪标签不稳定且不准确，导致性能下降。现有的视觉语言模型在医学图像上效果有限，因为领域特定的提示词难以迁移

Method: 提出免训练的伪标签生成和标签细化框架：1) 利用基于外观的简单描述（如"暗色椭圆形"）实现自然图像到医学图像的结构迁移，让视觉语言模型生成结构一致的伪标签；2) 用这些伪标签预热静态教师模型，捕获乳腺病灶的全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别

Result: 在四个乳腺超声数据集上的实验表明，该方法在仅使用2.5%标注数据的情况下，性能达到与全监督模型相当的水平，显著优于现有的半监督学习方法

Conclusion: 该方法为有限标注下的可扩展半监督医学图像分割提供了新范式，仅需全局外观描述即可获得可靠的伪监督，可扩展到其他成像模态或疾病
...
TL;DR: JOPP-3D是一个开放词汇的语义分割框架，通过联合利用全景图像和点云数据，实现语言驱动的场景理解，在2D和3D开放与封闭词汇语义分割任务上显著超越现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 3D点云和全景图像等视觉模态的语义分割面临两大挑战：标注数据稀缺和固定标签模型适应性有限。现有方法难以处理开放词汇查询和跨模态语义理解。

Method: 将RGB-D全景图像转换为对应的切向透视图像和3D点云，利用这两种模态提取和对齐基础视觉语言特征，通过自然语言查询生成两种输入模态的语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够生成跨全景和3D域的一致且语义有意义的分割结果，在开放和封闭词汇的2D和3D语义分割任务上相比现有最佳方法有显著提升。
...
TL;DR: 该论文提出了一种使用强化学习（RL）和多尺度反馈增强3D扩散模型的方法，通过预训练和PPO微调，结合2D切片和3D体积分析，在医学图像生成中实现局部细节和全局结构的同时优化。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在3D医学图像生成中表现出强大能力，但标准训练目标与临床相关性之间存在差距。需要开发能够同时优化局部纹理细节和全局结构一致性的方法，以提高生成图像的质量和临床实用性。

Method: 方法分为两个阶段：首先在MRI体积数据上预训练3D扩散模型建立生成先验；然后使用近端策略优化（PPO）进行微调，采用新颖的奖励系统，该系统整合了2D切片级评估和3D体积分析，形成多尺度反馈机制。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布发展。定量分析显示Fréchet Inception Distance（FID）显著改善，合成数据在肿瘤和疾病分类任务中表现出比非优化基线更高的实用性。

Conclusion: 通过强化学习与多尺度反馈的结合，成功提升了3D扩散模型在医学图像生成中的性能，不仅改善了图像质量指标，更重要的是增强了合成数据在下游临床任务中的实用价值。
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器中的两个关键问题，使生成能力更好地转化为分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练免费的扩散分割方法依赖交叉注意力图，但未能随更强大的扩散模型而有效扩展。研究发现两个根本问题：1）跨多头和多层的注意力图与统一全局表示存在差异；2）全局图无法直接转化为准确的分割语义相关性，因为不同文本标记存在分数不平衡。

Method: 提出两种技术：1）自动聚合（auto aggregation）：解决多头多层注意力图与全局表示的差异问题；2）逐像素重缩放（per-pixel rescaling）：解决不同文本标记分数不平衡问题，使全局图能准确反映语义相关性。

Result: 在标准语义分割基准上评估了该方法，并进一步集成到生成技术中，展示了改进的性能和广泛的适用性。

Conclusion: 通过自动聚合和逐像素重缩放两种技术，成功解决了训练免费扩散分割器中的关键问题，使生成能力能更好地转化为分割性能，提高了方法的可扩展性和实用性。
...
TL;DR: 该论文提出了一种基于骨架时序动作分割的点监督框架，仅需每个动作片段标注单帧，通过多模态骨架数据、原型相似性方法和伪标签集成技术，在显著减少标注成本的同时达到与全监督方法相竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 全监督的骨架时序动作分割方法需要昂贵的逐帧标注，且对模糊的动作边界敏感。为降低标注成本并解决边界模糊问题，研究者提出点监督框架，只需每个动作片段标注单帧。

Method: 1. 利用预训练统一模型编码多模态骨架数据（关节、骨骼、运动信息）提取丰富特征表示；2. 提出新颖的原型相似性方法，并与能量函数和约束K-Medoids聚类两种现有方法结合生成可靠伪标签；3. 提出多模态伪标签集成技术，增强伪标签可靠性并指导模型训练。

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立了新的点监督基准，实验表明该方法在显著减少标注工作量的同时，达到了与全监督方法相竞争的性能，甚至在某些情况下超越了全监督方法。

Conclusion: 提出的点监督框架有效解决了骨架时序动作分割中的高标注成本和边界模糊问题，通过多模态特征提取和伪标签集成技术，在减少标注工作量的同时保持了高性能，为智能系统感知和响应人类活动提供了更实用的解决方案。
...
TL;DR: VG3S提出了一种新颖的视觉几何基础高斯溅射框架，通过注入视觉基础模型的几何感知能力来提升3D语义占据预测性能，在nuScenes基准上实现了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯溅射的占据预测方法虽然降低了计算开销，但高质量3D高斯的生成严重依赖准确的几何线索，这在纯视觉中心范式中往往不足。需要将视觉基础模型强大的几何基础能力注入到占据预测中。

Method: 提出VG3S框架，通过分层几何特征适配器充分利用冻结视觉基础模型的丰富3D几何先验。该适配器通过特征聚合、任务特定对齐和多尺度重构，有效转换通用VFM令牌，实现跨视图3D几何基础。

Result: 在nuScenes占据基准上的广泛实验表明，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。框架能够无缝泛化到不同的视觉基础模型，一致提升占据预测准确性。

Conclusion: VG3S证明了将来自强大预训练几何基础视觉基础模型的先验整合到占据预测中的巨大价值，为自动驾驶场景理解提供了更全面的解决方案。
...
TL;DR: 提出了一种基于特征熵优化的邻域密度化策略EntON，用于3D高斯泼溅(3DGS)中的几何精确和高品质3D重建，通过交替优化框架结合几何感知的密度化来提升几何精度和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 标准3DGS产生的高斯中心与底层物体几何对齐不佳，而专注于表面的重建方法常常牺牲光度精度。需要一种能平衡几何精度和渲染质量的方法。

Method: 提出特征熵优化的邻域密度化策略EntON：1) 从每个高斯中心k近邻的协方差矩阵特征值计算3D形状特征熵，量化局部结构有序度；2) 在交替优化框架中结合标准梯度密度化和特征熵感知密度化；3) 优先在低特征熵（有序、平坦）区域密度化以捕捉精细几何细节，在高特征熵（无序、球形）区域修剪。
...
TL;DR: WAFL将时序伪造定位任务从回归和连续定位转变为离散词级二分类，通过词锚定、特征重对齐和不对称损失解决特征粒度不匹配和类别不平衡问题


<details>
  <summary>Details</summary>
Motivation: 现有时序伪造定位方法存在特征粒度不匹配和计算成本高的问题，需要一种更高效、更精确的解决方案

Method: 提出词锚定的时序伪造定位范式，基于最小有意义的伪造单元（词标记），引入法证特征重对齐模块将预训练语义特征映射到判别性法证流形，设计基于伪影的非对称损失解决类别不平衡

Result: WAFL在内外数据集设置下显著优于现有方法，定位性能更优，同时需要更少的可学习参数并具有更高的计算效率

Conclusion: 词锚定的二分类范式为时序伪造定位提供了更高效、更精确的解决方案，通过特征重对齐和不对称损失有效解决了现有方法的局限性
...
TL;DR: HCF-RES提出了一种多模态3D指代表达分割框架，通过分层视觉语义分解和渐进多级融合，在稀疏点云基础上增强视觉语义，显著提升了3D指代表达分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理细粒度描述。当描述匹配多个目标或零目标时，现有方法表现受限。

Method: 1. 分层视觉语义分解：利用SAM实例掩码指导CLIP编码，提取像素级和实例级双粒度特征，在2D到3D投影中保持对象边界。2. 渐进多级融合：通过模态内协作、2D语义与3D几何特征的跨模态自适应加权，以及语言引导的细化，集成多模态表示。

Result: HCF-RES在ScanRefer和Multi3DRefer两个基准测试中均取得了最先进的结果。

Conclusion: 通过引入分层视觉语义分解和渐进多级融合，HCF-RES有效解决了现有3D指代表达分割方法视觉语义不足的问题，显著提升了多目标匹配和细粒度描述的处理能力。
...
TL;DR: NOVA提出了一种新的开放词汇3D多目标跟踪范式，通过将轨迹重构为结构化时空语义序列，利用LLM的自回归能力实现跨未知目标的泛化跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有3D MOT方法受限于闭集假设和"语义盲"启发式方法，无法有效处理开放世界感知中的未知目标泛化问题。需要一种能够同时编码物理运动连续性和深度语言先验的新范式。

Method: NOVA将3D轨迹重构为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为原则性的下一步序列补全过程。通过语言空间的层次结构解决细粒度语义模糊性，并通过高层常识推理保持复杂长序列的身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上的实验表明NOVA具有优越性能。在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线有20.21%的绝对提升，仅使用0.5B参数的自回归模型实现。

Conclusion: NOVA通过生成式时空语义建模范式，成功将3D跟踪从传统的基于距离的碎片化匹配转向语义感知的序列生成，实现了开放词汇场景下的跨未知目标泛化能力，为开放世界感知提供了新思路。
...
TL;DR: StrSR是一种用于扩散变换器（DiT）图像超分辨率的单步对抗蒸馏框架，通过谱和轨迹正则化解决轨迹不匹配和网格状伪影问题
...
Result: 大量实验表明，StrSR在真实图像超分辨率任务中实现了最先进的性能，包括定量指标和视觉感知两方面

Conclusion: StrSR通过谱和轨迹正则化有效解决了DiT单步蒸馏中的关键问题，为真实图像超分辨率提供了高效且高质量的解决方案
...
TL;DR: 该论文提出OccNL基准和DPR-Occ框架，首次系统研究3D语义占据预测中的标签噪声问题，通过双源部分标签推理在极端噪声下保持性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的3D体素标注存在结构伪影和动态拖尾效应等固有噪声，这对机器人感知的安全性构成威胁。现有2D标签噪声学习方法在稀疏3D体素空间中会灾难性失效，暴露了现有范式的关键脆弱性。

Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督：1) 利用时间模型记忆捕捉动态一致性；2) 利用表示级结构亲和力识别空间语义关系。该框架动态扩展和修剪候选标签集，保留真实语义同时抑制噪声传播。

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下防止了几何和语义崩溃。即使在90%标签噪声下，相比现有标签噪声学习方法，实现了显著性能提升（最高2.57% mIoU和13.91% IoU）。

Conclusion: 通过建立OccNL基准和提出DPR-Occ框架，该研究填补了标签噪声学习与3D感知之间的空白，为动态环境中安全关键的机器人感知提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习通过合成未见类特征来迁移知识，但面临两个内在挑战：1) 类级属性无法捕捉实例特定的视觉外观，导致类-实例差距；2) 语义与视觉特征分布不匹配，表现为类间相关性差异，导致语义-视觉域差距。

Method: 提出ADiVA方法，包含两个模块：1) 属性分布建模模块学习每个类的可迁移属性分布，为未见类采样实例级属性；2) 视觉引导对齐模块细化语义表示以更好反映视觉结构。

Result: 在三个广泛使用的基准数据集上，ADiVA显著优于最先进方法（如在AWA2和SUN数据集上分别获得4.7%和6.1%的提升），且可作为插件增强现有生成式ZSL方法。

Conclusion: ADiVA通过联合建模属性分布和执行显式语义-视觉对齐，有效解决了生成式零样本学习中的类-实例差距和语义-视觉域差距问题，为ZSL领域提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖T2V模型中间输出构建运动指导，计算开销大且灵活性有限。本文旨在开发更高效灵活的运动迁移方法，直接利用流基T2V模型的预测输出。

Method: 提出FlowMotion框架：1）流指导：从早期潜在预测中提取运动表示，对齐源视频与生成视频的运动模式；2）速度正则化策略：稳定优化并确保平滑运动演化。完全基于模型预测操作，无需额外训练。

Result: 相比现有方法，FlowMotion在时间和资源效率上表现优异，同时保持有竞争力的性能。实现了高效灵活的视频运动迁移。

Conclusion: FlowMotion通过直接利用流基T2V模型的预测输出，提供了一种高效、灵活且无需训练的视频运动迁移解决方案，在计算效率和性能之间取得了良好平衡。
...
Result: 该方法能生成高质量、伪影减少的3D CBCT图像，证明其在提高临床成像质量方面的有效性。
...
Conclusion: 提出的小波增强专家混合提示学习方法能够有效解决零样本异常检测中固定提示的局限性，通过多频特征提取和语义建模提升了对复杂异常模式的检测能力。
...
Method: 提出原型库驱动的无监督点云语义分割策略P-SLCR：1) 一致性结构学习：通过选择高质量特征，在一致性点和一致性原型库之间建立结构特征学习；2) 语义关系一致性推理：分别构建一致性和模糊性原型库之间的原型相互关系矩阵，通过该矩阵对两种原型库施加约束以保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。特别是在S3DIS数据集的Area-5上实现了47.1%的mIoU，超过了经典全监督方法PointNet的2.5%。

Conclusion: P-SLCR方法通过原型库驱动的结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上超越了现有无监督方法，甚至在某些指标上优于全监督基准方法。
...
TL;DR: WorldCache：针对扩散世界模型的缓存框架，通过曲率引导的异构令牌预测和混沌优先自适应跳过技术，在保持98%推演质量的同时实现3.7倍端到端加速


<details>
  <summary>Details</summary>
Motivation: 基于扩散的世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程在交互使用和长时域推演中计算成本过高。现有特征缓存方法虽然可以加速推理，但由于令牌异构性和非均匀时间动态性这两个世界模型特有的障碍，为单模态扩散设计的策略无法有效迁移到世界模型中。
...
Result: 在扩散世界模型上的实验表明，WorldCache在保持98%推演质量的同时，实现了高达3.7倍的端到端加速，在资源受限场景下展现出显著优势和实用性。

Conclusion: WorldCache是针对扩散世界模型量身定制的缓存框架，通过解决令牌异构性和非均匀时间动态性问题，显著加速了世界模型的推理过程，为资源受限场景下的高效世界模拟提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D网格生成方法通常依赖等值面提取或启发式网格化，这些方法在保持复杂几何拓扑结构方面存在局限性，且推理效率不高。需要一种能够直接生成具有良好拓扑结构的显式3D网格的方法。

Method: 1. 使用顶点位移场(VDF)在表面上锚定网格表示；2. 采用稀疏体素变分自编码器(VAE)将显式信号压缩为结构化、拓扑感知的体素潜在表示；3. VAE解码器通过逐步细分和修剪潜在体素来实例化精确顶点位置；4. 专用连接头查询体素潜在表示以直接预测顶点对之间的边连接性；5. 采用两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征。

Result: 与先前的等值面/三角形扩散模型和自回归生成方法相比，LATO能够生成具有复杂几何形状、良好拓扑结构的网格，同时在推理时具有高效率。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示方法，实现了可扩展的流匹配式3D网格合成，能够直接生成显式网格而无需等值面提取或启发式网格化，在保持复杂拓扑结构的同时提高了生成效率。
...
TL;DR: OralGPT-Plus：一种用于全景牙科X光片分析的智能视觉语言模型，采用迭代对称感知诊断推理，通过DentalProbe数据集和Reinspection强化学习框架提升临床可靠性
...
Method: 1. 构建DentalProbe数据集（5000张图像，包含专家标注的诊断轨迹）；2. 开发Reinspection驱动的强化学习框架，结合基于规则的奖励和条件诊断驱动奖励；3. 提出OralGPT-Plus模型，支持迭代对称感知诊断推理

Result: OralGPT-Plus在MMOral-X基准测试（首个全景诊断基准，包含300个开放式问题和区域级标注）和现有全景基准上均表现出优于强基线的持续可靠改进

Conclusion: 智能建模对牙科影像分析具有重要价值，OralGPT-Plus为临床对齐的全景X光片分析提供了基础，展示了交互式和对称感知推理的有效性
...
<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注成本高昂，稀疏标注作为弱监督替代方案仍存在性能差距，需要利用3D场景重建提供额外监督信号

Method: 提出利用3D场景重建作为辅助监督信号的新方法，通过双学生-教师架构在2D图像和重建的3D点云之间强制语义一致性，使用SOTA前馈重建生成可靠的几何监督

Result: Rewis3d在稀疏监督下达到最先进性能，比现有方法提升2-7%，无需额外标注或推理开销

Conclusion: 3D几何结构从2D视频中恢复的几何线索能有效传播稀疏标注，显著提升弱监督语义分割性能
...
<details>
  <summary>Details</summary>
Motivation: 准确的冠层高度信息对于量化森林碳储量、监测恢复与退化以及评估栖息地结构至关重要，但全球范围内高保真的机载激光扫描(ALS)测量数据分布不均。

Method: 使用基于DINOv3的深度估计模型，从高分辨率光学卫星影像生成冠层高度图；通过大规模地理多样性训练数据扩展、自动化数据整理与配准，以及针对冠层高度分布定制的损失函数和数据采样策略实现改进。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，并更好地保留了冠层边缘和间隙等精细尺度结构；在主要森林生物群落中表现出稳定性能。

Conclusion: CHMv2通过结合大规模训练数据、自动化数据处理和针对性算法设计，提供了全球范围内高精度、米级分辨率的冠层高度图，为森林监测和生态研究提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法往往只专注于单一目标：要么提升关系预测准确率，要么增强物体检测准确率，要么降低延迟，而缺乏同时平衡这三个目标的综合方案。为了支持实时应用，需要在性能与推理速度之间取得更好的权衡。

Method: 基于REACT架构，采用高效特征提取和原型空间中的主体到客体交叉注意力机制，在保持计算效率的同时增强关系表示能力。
...
TL;DR: 提出一种人机协同的考古碎片重组框架，通过结合自动求解器和人类交互指导，解决真实世界文物碎片重组中的复杂性和规模问题。


<details>
  <summary>Details</summary>
Motivation: 真实世界考古文物碎片重组面临侵蚀、缺失区域、不规则形状和大规模模糊性等挑战，传统拼图求解器在合成场景中表现良好，但在真实文物重组中，特别是当碎片数量达到数千片时（如RePAIR基准），难以有效处理。

Method: 提出人机协同拼图求解框架，整合自动松弛标记求解器与交互式人工指导。用户可迭代锁定已验证的放置位置、纠正错误，并引导系统实现语义和几何一致的组装。引入两种互补的交互策略：迭代锚定和连续交互细化，支持在不同模糊度和拼图规模下的可扩展重建。

Result: 在多个RePAIR组上的实验表明，该混合方法在准确性和效率上显著优于完全自动化和手动基线，为大规模专家参与的文物重组提供了实用解决方案。

Conclusion: 人机协同框架通过结合自动求解器的计算能力和人类专家的领域知识，有效解决了真实世界考古文物碎片重组中的复杂性和规模挑战，为文化遗产重建提供了可行的技术路径。
...
TL;DR: DiffInf是一个基于自影响力引导的扩散框架，用于缓解面部属性学习中的标注不一致问题，通过生成式校正而非丢弃样本来改善数据集质量，提升下游分类性能。


<details>
  <summary>Details</summary>
Motivation: 面部属性分类依赖于大规模标注数据集，但许多属性（如年龄、表情）本质上是模糊且连续的，却被离散化为分类标签。标注不一致源于主观性和视觉混淆因素（如姿态、光照、表情、人口统计变异），导致图像与分配标签不匹配。这些不一致引入了监督错误，损害表示学习并降低下游预测性能。

Method: 1. 训练基线分类器并计算样本级自影响力分数（使用一阶近似）来识别对优化过程有过度影响的训练实例；2. 通过潜在扩散自编码器对高影响力样本进行针对性生成校正，使视觉内容与分配标签更好对齐，同时保持身份和真实感；3. 训练轻量级高影响力成员预测器作为替代影响力正则化器，在校正过程中提供可微指导；4. 用编辑后的样本替换原始样本，生成大小不变的影响力精炼数据集。

Result: 在多类别面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响力的过滤方法，持续改善了泛化性能。结果表明，在图像层面修复有影响力的标注不一致，能够在不牺牲分布覆盖的情况下增强下游面部属性分类。

Conclusion: 通过生成式校正而非丢弃样本的方法来缓解标注不一致问题，可以有效提升面部属性分类性能。DiffInf框架展示了修复有影响力的标注不一致在图像层面的有效性，为处理标注噪声提供了新思路。
...
TL;DR: BEiT视觉Transformer在感知冲突中优先选择凸性完成，研究发现这一偏好由特定注意力头(L0H9)驱动，调节该头可改变感知决策边界
...
Method: 使用基于飞镖形状合成图像的受控感知冲突实验，系统性地掩蔽既支持凹性完成也支持凸性完成的区域；通过logit归因将内部激活投影到离散视觉词表空间，识别影响感知偏好的功能单元；通过分解注意力头的直接影响，定位关键注意力头

Result: BEiT在感知冲突中可靠地偏好凸性完成；图形-背景组织在早期和中间层保持歧义，在深层突然解决；识别出L0H9注意力头作为早期种子，引入对凸性的弱偏置；下调该单个注意力头可连续改变感知决策边界，使凹性证据引导完成

Conclusion: BEiT中的格式塔凸性先验由特定可识别的功能单元实现，特别是L0H9注意力头作为早期种子；通过调节单个注意力头可连续控制感知决策，表明Transformer架构中的感知组织具有模块化和可调节的特性
...
<details>
  <summary>Details</summary>
Motivation: 在现实场景中，由于机器人物理限制或视障用户安全考虑，主动探索往往不可行。需要让智能体仅凭有限观察就能在心理上模拟到达目标情境的未来轨迹，并回答空间假设问题。

Method: 构建了WanderDream数据集：WanderDream-Gen包含15.8K个全景视频，覆盖1,088个真实场景；WanderDream-QA包含158K个问答对，涵盖轨迹的起始状态、路径和结束状态。通过世界模型和多模态大语言模型进行实验验证。

Result: 实验表明：(1)心理探索对情境推理至关重要；(2)世界模型在WanderDream-Gen上表现出色；(3)想象力显著促进WanderDream-QA上的推理能力；(4)WanderDream数据在真实场景中具有出色的可迁移性。

Conclusion: WanderDream是首个用于心理探索模拟的大规模数据集，使模型能够在无需主动探索的情况下进行推理，为解决现实世界中的空间推理问题提供了有效工具。
...
TL;DR: CaTok是一种因果图像分词器，通过MeanFlow解码器将2D图像转换为因果1D序列，支持快速单步生成和高保真多步采样，在ImageNet重建上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器存在两个主要问题：1）将2D图像块展平为非因果序列，或采用启发式排序与"下一token预测"模式不对齐；2）扩散自编码器要么缺乏因果性，要么引入不平衡。需要开发真正的因果视觉分词器来支持自回归语言模型范式。

Method: 提出CaTok因果图像分词器，核心包括：1）在时间间隔上选择token并与MeanFlow目标绑定，学习因果1D表示；2）MeanFlow解码器支持快速单步生成和高保真多步采样；3）提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐，稳定和加速训练。

Result: 在ImageNet重建任务上达到最先进结果：FID 0.75、PSNR 22.53、SSIM 0.674，且训练轮次更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功将因果分词范式扩展到视觉领域，解决了现有方法的局限性，为视觉自回归模型提供了有效的因果表示基础，在图像重建和生成任务上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，无法满足具有严格产品要求的用例。需要构建一个能够支持Pinterest图像编辑和增强用例的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，获得广泛的图像编辑能力。然后针对特定下游任务，在任务特定数据集上快速微调基础模型变体，为每个用例生成专门的模型。系统包括数据集管理、训练和推理的最佳实践。

Result: 在线A/B实验显示，增强图像分别获得18.0%和12.5%的显著参与度提升。与人工评分者比较进一步验证了模型在这些任务上优于第三方模型。系统还展示了多图像场景合成和图像到视频生成等变体。

Conclusion: Pinterest Canvas通过基础模型训练加任务特定微调的方法，能够有效支持各种图像编辑和增强用例，显著提升用户参与度，并可推广到多种潜在下游任务。
...
TL;DR: 该论文系统研究去噪生成模型的训练目标，重点分析损失加权和输出参数化（噪声、干净图像、速度）如何与数据流形内在维度、模型架构和数据集大小相互作用，旨在为流匹配模型提供实用设计指导。


<details>
  <summary>Details</summary>
Motivation: 去噪生成模型（如扩散模型、流匹配）的训练目标设计存在多种选择（损失加权、输出参数化），但这些选择如何与数据特性、模型架构等因素相互作用尚不明确。研究旨在系统分析这些因素，为实际训练提供指导而非提出新方法。

Method: 通过系统数值研究，在合成数据集（可控几何）和图像数据上比较不同训练目标：1）分析损失加权方案；2）比较噪声、干净图像和速度三种输出参数化；3）考察与数据流形内在维度、模型架构、数据集大小的相互作用；4）使用PSNR（去噪精度）和FID（生成质量）作为评估指标。

Result: 实验揭示了训练目标选择与数据特性之间的重要相互作用：1）最优损失加权方案取决于数据流形内在维度；2）不同输出参数化在不同场景下表现各异；3）模型架构和数据集大小影响训练目标的有效性；4）为不同应用场景提供了具体的设计建议。

Conclusion: 去噪生成模型的训练目标设计不是一刀切的解决方案，需要根据数据流形内在维度、模型架构和数据集大小进行针对性选择。研究为流匹配模型的实际训练提供了系统指导，强调了考虑数据几何特性的重要性。
...
TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督设计而非暴力扩展，在单24GB GPU上24小时内达到SOTA性能，挑战了"规模至上"的范式。


<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型(RFMs)开发过度依赖暴力扩展，直接移植自然图像处理方法，导致模型脆弱且计算成本高昂，难以在临床实践中应用。需要一种资源高效的方法来开发稳健且可推广的模型。

Method: 提出GreenRFM框架，采用MUST(More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning)监督设计原则，最大化利用监督信号而非单纯增加训练数据量。提供两种配置：性能模型(单24GB GPU 24小时)和轻量模型(6GB VRAM 4小时)。

Result: 在超过20万张来自4个机构、2种模态的图像上进行广泛实验。在胸部和腹部CT数据集上，无论公开还是私有基准，均超越多种基线模型。在内部肌肉骨骼MRI图像上的结果证明监督原则可在不同模态间迁移。计算需求降低数个数量级。

Conclusion: GreenRFM挑战了"规模至上"的教条，通过资源高效的预训练框架实现了SOTA性能，使临床医生即使在笔记本电脑上也能公平地开发最先进的放射学基础模型。
...
TL;DR: Match4Annotate：基于隐式神经表示的轻量级视频标注传播框架，支持点与掩码标注的跨视频传播


<details>
  <summary>Details</summary>
Motivation: 专业领域（如医学影像）中逐帧视频标注成本高昂，现有标签传播方法存在局限：视频跟踪器需要逐视频初始化且无法跨视频泛化；传统对应方法在低纹理场景表现不佳；密集特征匹配和单次分割方法缺乏时空平滑性且不支持点与掩码的统一处理。

Method: 提出Match4Annotate框架：1）测试时拟合SIREN隐式神经表示到DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场以指导对应匹配；3）支持点与掩码标注的跨视频传播。

Result: 在三个临床超声数据集上评估：Match4Annotate在跨视频传播方面达到最先进水平，优于特征匹配和单次分割基线；在视频内传播方面与专用跟踪器竞争力相当。

Conclusion: 轻量级、测试时优化的特征匹配流程为可扩展标注工作流提供了高效且易用的解决方案，展示了隐式神经表示在视频标注传播中的潜力。
...
TL;DR: Self-Flow是一种自监督流匹配范式，通过双时间步调度机制在生成框架内集成表示学习，无需外部模型即可同时学习强语义表示和生成能力
...
TL;DR: AV-Unified是一个统一的音频-视觉场景理解框架，通过将不同任务的输入输出标准化为离散token序列，实现跨任务的联合学习，包含多尺度时空感知模块和跨模态指导机制。


<details>
  <summary>Details</summary>
Motivation: 当前音频-视觉场景理解任务（如事件定位、解析、分割和问答）大多独立研究，难以全面理解复杂场景和探索任务间关系，需要统一的框架来整合这些任务。

Method: 1) 将所有任务的输入输出标准化为离散token序列，建立共享表示；2) 设计多尺度时间感知模块捕捉不同时间粒度的音频-视觉事件；3) 设计基于跨模态指导的空间感知模块建模空间音频-视觉关联；4) 使用任务特定文本提示增强模型适应性和任务感知能力。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS和AVS）上的广泛实验证明了AV-Unified在时间、空间和时空任务上的有效性。

Conclusion: AV-Unified通过统一框架实现了跨多种音频-视觉场景理解任务的联合学习，标准化表示和多尺度时空感知设计使其能够全面理解复杂音频-视觉场景。
...
TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间标定方法，通过扫描反光贴片估计每个LiDAR像素在RGB图像平面中的有效区域和相对空间灵敏度


<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间LiDAR通过聚合宽视场内的光子返回形成深度直方图，违反了标准LiDAR-RGB标定的单射线假设，需要新的空间标定方法

Method: 使用扫描的反光贴片结合背景减法，恢复每个LiDAR像素的响应图，估计其在共位RGB图像平面中的足迹（有效支持区域）和相对空间灵敏度

Result: 该方法在ams OSRAM TMF8828上得到验证，获得了明确的LiDAR到RGB对应关系，可用于跨模态对齐和融合

Conclusion: 提出的简单空间标定程序能够有效解决漫反射LiDAR与RGB相机之间的空间对应问题，为多模态感知系统提供了实用的标定解决方案
...
Method: 将语言否定建模为扩散动态中语义引导的结构化可行性约束。将无分类器引导重新解释为定义语义更新方向，并通过将更新投影到从语言结构导出的凸约束集来强制执行否定。该方法无需训练，与预训练的扩散主干兼容。

Result: 方法能够处理多样化的否定现象，包括对象缺失、分级非反转语义、多否定组合和范围敏感消歧。在图像生成和时间演化的视频轨迹中都表现出色，实现了稳健的否定遵从性，同时保持了视觉保真度和结构连贯性。

Conclusion: 建立了扩散基生成模型中语言否定的第一个统一表述，超越了表示级评估，并引入了结构化的否定中心基准套件来隔离生成系统中的不同语言失败模式。
...
TL;DR: 该研究分析了自动驾驶数据集中冗余性作为数据质量因素的影响，通过实验证明选择性移除冗余标签可以提升目标检测性能


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过于关注算法设计而忽视数据质量分析，特别是多源多模态数据中的冗余性问题尚未得到充分探索，而数据质量变化会影响实时决策性能

Method: 使用nuScenes和Argoverse 2数据集，建模和测量多源摄像头数据和多模态图像-LiDAR数据的冗余性，评估移除冗余标签对YOLOv8目标检测任务的影响

Result: 选择性移除共享视野摄像头中的冗余多源图像目标标签可提升检测性能：nuScenes中三个代表性重叠区域的mAP50分别从0.66提升至0.70、0.64至0.67、0.53至0.55；AV2中移除4.1-8.6%标签后mAP50保持在0.64基线附近；多模态分析显示图像和LiDAR数据间存在显著冗余

Conclusion: 冗余性是自动驾驶数据集中可测量且可操作的数据质量因素，直接影响性能表现，强调了以数据为中心的视角来评估和改进自动驾驶数据集的重要性
...
TL;DR: EgoReasoner：针对自我中心视频理解的4D推理任务，提出两阶段框架，通过任务自适应思维模板和任务感知奖励函数，在少量数据上显著提升推理性能


<details>
  <summary>Details</summary>
Motivation: 自我中心视频理解面临动态4D环境的复杂性，现有任务无关方法在处理空间锚定、时间跟踪和持续时间推理等不同认知操作时表现不足，需要针对任务认知结构进行专门设计

Method: 提出EgoReasoner两阶段框架：1）任务自适应思维模板指导合成结构化思维链轨迹，通过监督微调使模型适应不同任务类型；2）任务感知奖励函数验证实体基础、时间对齐和任务自适应逻辑一致性，通过GRPO进行强化微调选择性强化推理路径

Result: 仅用16K样本训练的3B参数模型在HD-EPIC基准测试中达到37.5%平均准确率，显著超越Qwen2.5-VL-7B（25.7%）超过10个百分点

Conclusion: 通过将推理框架和奖励信号与任务认知结构对齐，EgoReasoner能够有效处理自我中心视频理解中的复杂4D推理任务，在少量数据上实现显著性能提升
...
TL;DR: Penguin-VL提出了一种轻量级视觉语言模型，通过使用文本LLM初始化的视觉编码器替代传统的对比预训练视觉编码器，在保持小模型规模的同时实现了与大型VLM相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型(VLM)的发展主要依赖模型规模扩展，这阻碍了在计算受限的移动和边缘设备上的部署。同时，主流VLM依赖通过大规模对比预训练(如CLIP/SigLIP)初始化的视觉编码器，存在目标不匹配问题：对比学习优化的判别性任务会抑制细粒度视觉线索，而这些线索对于密集描述和复杂VLM推理至关重要。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而不是传统的对比预训练视觉编码器。这种方法解决了对比学习带来的目标不匹配问题，能够保留更精细的空间和时间线索。模型采用轻量级架构(如2B和8B参数规模)，专注于改进视觉表示而非模型扩展。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理任务上达到与领先VLM(如Qwen3-VL)相当的性能，在文档理解、视觉知识和多视角视频理解等任务上超越它们。Penguin-Encoder在消融实验中始终优于对比预训练的编码器，保留了密集感知和复杂推理所需的关键细粒度线索。

Conclusion: 研究表明，改进视觉表示而非模型扩展是性能提升的主要驱动力。Penguin-Encoder作为传统对比预训练的优越替代方案，为计算高效的VLM提供了强大的即插即用选择，能够在资源受限的环境中实现高性能。这为在移动和边缘设备上部署高效VLM开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用传统的自回归架构，在架构设计上存在探索有效且高效替代方案的空间。同时，离散扩散模型在视觉理解和图像生成等领域显示出作为多模态系统骨干的巨大潜力。

Method: 采用统一的掩码离散扩散模型直接捕捉离散多模态标记的联合分布，支持双模态任务以及涉及多个模态的更复杂场景。

Result: 在多样化基准测试中，该方法优于或与处理两个或更多模态的现有多模态系统表现相当，突显了扩散模型在驱动下一代多模态基础模型方面的巨大潜力。

Conclusion: Omni-Diffusion展示了基于扩散模型构建多模态系统的可行性，为下一代多模态基础模型提供了有前景的架构替代方案。
...
TL;DR: 该研究揭示了多模态大语言模型（MLLM）分类性能评估中的关键问题：评估协议和标注质量严重影响结果，许多报告的MLLM性能不足实际上是噪声标注和有缺陷评估协议的产物，而非模型本身的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有研究在比较MLLM与监督学习和视觉语言模型时得出相互矛盾的结论，这些矛盾源于评估协议要么夸大要么低估性能。需要识别并修复评估中的关键问题，准确评估MLLM的真实分类能力。

Method: 1) 识别并修复常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱多选题干扰项导致结果膨胀、开放世界设置因输出映射不佳而表现不佳；2) 量化常被忽视的设计选择影响：批量大小、图像顺序、文本编码器选择；3) 使用ReGT（ImageNet-1k 625个类别的多标签重新标注）进行评估；4) 进行受控案例研究，评估MLLM辅助人类标注者的潜力。

Result: 1) MLLM从修正标注中获益最大（提升达+10.8%），显著缩小了与监督模型的感知差距；2) 许多报告的MLLM分类性能不足是噪声标注和有缺陷评估协议的产物；3) 较少依赖监督训练信号的模型对标注质量最敏感；4) MLLM可辅助人类标注者：在困难案例中，标注者确认或整合了约50%的MLLM预测，展示了大规模数据集整理的潜力。

Conclusion: MLLM的分类性能被现有评估协议和标注质量问题严重扭曲。通过修正标注和评估协议，MLLM与监督模型的性能差距显著缩小。MLLM不仅不是分类任务的弱模型，反而对标注质量敏感，并能有效辅助数据集整理工作。该研究是"Aiming for Perfect ImageNet-1k"项目的一部分。
...
TL;DR: Aletheia是一个基于RAG和LLM的浏览器扩展，用于假新闻检测并提供证据解释，包含讨论中心和最新事实核查功能，在检测性能和用户体验方面均优于现有方法。
...
Method: 提出Aletheia浏览器扩展，采用检索增强生成(RAG)和大语言模型(LLM)进行假新闻检测并提供基于证据的解释；系统包含两个交互组件：讨论中心支持用户围绕标记内容进行对话，"保持知情"功能展示最新事实核查。

Result: 实验表明Aletheia在检测性能上优于最先进的基线方法；补充性用户研究(250名参与者)证实了系统的可用性和感知有效性，突显其作为透明工具对抗在线假新闻的潜力。

Conclusion: Aletheia通过结合RAG、LLM和交互功能，提供了一个有效、透明、用户中心的假新闻检测解决方案，在技术和用户体验层面均表现出色，有望成为对抗在线虚假信息的有力工具。
...
TL;DR: 本文研究语法约束解码(GCD)，证明语法等价性不影响下一个令牌的掩码但影响编译状态空间和在线歧义成本，引入结构歧义成本(SAC)度量，建立引擎无关的下界，定义解码成本等价类，推导硬掩码解码的KL散度和总变差失真界限，并与Transformer和MoE架构集成。


<details>
  <summary>Details</summary>
Motivation: 研究语法约束解码中语法等价性对解码效率的影响，特别是虽然等价语法产生相同的下一个令牌掩码，但编译状态空间和在线计算成本可能显著不同，这在实际应用中影响解码延迟和资源消耗。

Method: 将GCD建模为自回归下一个令牌分布与从上下文无关语法编译的下推系统可达性预言机的耦合；证明预言机不变性定理；引入左到右结构歧义成本(SAC)度量增量打包解析森林增长；建立引擎无关下界；定义解码成本等价类；推导硬掩码解码的KL散度和总变差失真界限。

Result: 证明语言等价语法在每个前缀处产生相同的可接受下一个令牌集和logit掩码；给出规范a^n b^n语言在冗余非终结符委托下的精确控制状态爆炸计数；SAC在右递归下为O(1)每令牌，在连接下为Θ(t^2)每令牌和Θ(n^3)累计；任何在线掩码引擎必须在特定CFG族上每令牌执行Ω(t^2)工作；存在有界重写族中的最小SAC代表；推导硬掩码解码的KL散度和总变差失真界限。

Conclusion: 语法约束解码中语法等价性不影响掩码但显著影响计算效率，结构歧义成本是解码延迟的关键因素，存在最小SAC语法代表，硬掩码解码引入的分布失真可通过生存概率扩散量化，这些结果与Transformer和MoE架构集成，为自动化语法优化提供理论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器生成文本检测方法缺乏可解释性，用户难以理解检测决策的依据。需要开发一个既能准确检测又能提供透明解释的框架，帮助用户理解AI生成文本的特征信号。

Method: 扩展Fast-DetectGPT方法，整合17个可解释特征：条件概率曲率、ModernBERT检测器分数、可读性指标和文体特征。使用梯度提升树（XGBoost）作为元分类器，应用SHAP进行特征归因，并通过LLM生成自然语言解释。

Result: 开发了NOTAI.AI系统，包含交互式Web应用程序，支持实时分析、可视化特征检查和结构化证据展示。系统公开了源代码和演示视频，确保可复现性。

Conclusion: NOTAI.AI提供了一个可解释的机器生成文本检测框架，通过多特征融合和透明解释机制，增强了检测系统的可信度和用户理解能力。
...
TL;DR: CoT提示增强LLM推理但增加PII泄露风险，研究提出模型无关框架量化泄露，发现CoT持续增加高风险类别泄露，泄露程度依赖模型家族和推理预算，评估多种轻量级门控方法但无单一最优方案


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought (CoT)提示虽然能提升大型语言模型的推理能力，但会增加隐私风险，可能将提示中的个人身份信息(PII)泄露到推理轨迹和输出中，即使模型被指示不要重述PII。需要系统研究这种推理时的直接PII泄露问题。

Method: 提出模型无关框架：(1)将泄露定义为11种PII类型的风险加权、令牌级事件；(2)追踪泄露曲线作为允许CoT预算的函数；(3)在结构化PII数据集上比较开源和闭源模型家族，使用分层风险分类法。评估四种轻量级推理时门控方法：基于规则的检测器、TF-IDF+逻辑回归分类器、GLiNER-based NER模型和LLM-as-judge，使用风险加权F1、Macro-F1和召回率指标。

Result: CoT持续增加泄露风险，特别是高风险类别；泄露程度强烈依赖模型家族和推理预算；增加推理预算可能放大或减弱泄露，取决于基础模型；没有单一门控方法在所有模型或预算上表现最优，需要混合、风格自适应的门控策略来平衡效用和风险。

Conclusion: CoT提示确实增加PII泄露风险，需要系统评估框架和混合门控策略。研究建立了可复现的通用协议，为平衡推理效用和隐私保护提供了实证基础，强调需要根据模型特性和推理预算制定自适应隐私保护机制。
...
TL;DR: LLM道德判断易受叙述视角和评估协议影响，而非仅基于道德实质，引发可重复性和公平性担忧


<details>
  <summary>Details</summary>
Motivation: 随着人们越来越多地使用LLM获取道德和人际指导，但这些系统无法审视缺失的上下文，且对呈现方式的判断存在不稳定性，需要测试LLM道德判断的稳定性和可操纵性

Method: 使用2,939个来自r/AmItheAsshole的道德困境案例，生成三类内容扰动：表面编辑（词汇/结构噪声）、视角转换（声音和立场中立化）、说服线索（自我定位、社会证明、模式承认、受害者框架）。同时改变评估协议（输出顺序、指令位置、非结构化提示）。使用四个模型（GPT-4.1、Claude 3.7 Sonnet、DeepSeek V3、Qwen2.5-72B）评估所有变体，共129,156个判断

Result: 表面扰动产生低翻转率（7.5%），基本在自一致性噪声范围内（4-13%）；而视角转换导致显著更高的不稳定性（24.3%）。37.9%的困境对表面噪声稳健但在视角变化下翻转，表明模型将叙述声音作为实用线索。不稳定性集中在道德模糊案例中；无人被指责的场景最易受影响。说服扰动产生系统性方向性偏移。协议选择主导所有其他因素：结构化协议间一致性仅67.6%（kappa=0.55），仅35.7%的模型-场景单元在所有三种协议中匹配
...
TL;DR: FreeTxt-Vi是一个免费开源的基于Web的双语越南语-英语文本分析工具包，结合语料库语言学和NLP技术，提供文本分析功能而无需编程技能，通过统一的NLP流水线在多个任务上达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 开发一个免费开源的工具包，降低多语言文本分析的技术门槛，特别是针对越南语这种在NLP中广泛使用但代表性不足的语言，支持可重复研究并促进越南语语言资源的发展。

Method: 设计统一的双语NLP流水线，集成混合VnCoreNLP和Byte Pair Encoding（BPE）分词策略、微调的TabularisAI情感分类器和微调的Qwen2.5模型进行抽象摘要。系统结合语料库分析功能（如共现分析、关键词分析、词关系探索和交互式可视化）与基于Transformer的NLP组件。

Result: 通过三部分评估（分词、情感分析和摘要生成），显示该方法在越南语和英语上都达到了与广泛使用的基线方法竞争或更优的性能。工具包在教育、数字人文、文化遗产和社会科学等领域具有应用价值。

Conclusion: FreeTxt-Vi通过降低多语言文本分析的技术障碍，支持可重复研究并促进越南语语言资源的发展，为广泛使用但代表性不足的语言提供了实用的文本分析解决方案。
...
TL;DR: 该研究使用RGB基准评估RAG系统在噪声、信息整合、负拒绝和反事实场景下的鲁棒性，比较了RGB基线方法与GraphRAG知识图谱检索系统，测试了三种GraphRAG定制化方案并展示了改进效果。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然能增强LLM的能力并减少事实幻觉，但检索信息的不一致性会影响LLM响应质量。需要评估RAG系统在不同挑战性场景下的鲁棒性，特别是比较传统RAG与知识图谱方法的性能差异。

Method: 使用RGB基准在四个场景下评估RAG系统：噪声鲁棒性、信息整合、负拒绝和反事实鲁棒性。比较RGB RAG基线与GraphRAG知识图谱检索系统，测试了三种GraphRAG定制化方案来提升鲁棒性。

Result: GraphRAG在RGB基准上相比基线方法表现出改进，三种定制化方案进一步提升了系统鲁棒性，为设计更可靠的现实世界RAG系统提供了重要见解。

Conclusion: 知识图谱基础的GraphRAG系统在应对不一致检索信息方面比传统RAG方法更具优势，定制化方案能进一步提升鲁棒性，这对构建可靠的实际应用RAG系统具有指导意义。
...
TL;DR: 该研究通过全球调查评估了不同文化群体对GenAI应如何呈现文化的态度和信念，提出了基于社区参与的文化AI发展建议。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏关于全球对GenAI应如何呈现文化的态度的实证证据，需要了解不同群体对文化的理解以及GenAI应如何处理文化表征。

Method: 采用大规模全球调查方法，收集欧洲、北美、南美、亚洲和非洲地区的数据，直接从社区获取文化的工作定义，分析文化概念复杂性及其与生成式AI表征的关系。

Result: 从调查中提炼出文化的操作性定义，揭示了文化概念的复杂性及其与AI表征的关系，形成了关于GenAI文化发展的具体建议。

Conclusion: 提出了一套文化AI发展建议：采用参与式方法，优先考虑超越地理的特定文化维度（如宗教和传统），并建立处理文化"红线"的敏感性框架。
...
TL;DR: 提出基于L-积的张量Transformer架构，通过谱分解将嵌入空间分解为p个独立谱子变换器，在保持Transformer语义的同时实现约1/p的编码器参数减少


<details>
  <summary>Details</summary>
Motivation: Transformer架构在扩展时伴随参数增长和嵌入维度冗余问题，需要更高效的参数利用方法

Method: 基于三阶张量L-积的结构化谱分解，将token表示重塑为谱张量切片，在变换域执行注意力与前馈操作，构建张量Transformer架构

Result: 在IMDB和AG News数据集上，模型可减少高达75%编码器参数(p=4)并保持竞争性准确率；在BERT-base宽度下性能恢复至基准水平

Conclusion: 提出的L-Transformer通过谱分解实现参数高效压缩，同时引入嵌入频率的归纳偏置，改善泛化能力，为Transformer架构压缩提供新方法
...
TL;DR: 该立场论文以瓜拉尼语为案例，指出AI系统普遍存在的文本优先设计问题，并提出一种面向口语实践的多智能体架构作为替代方案。


<details>
  <summary>Details</summary>
Motivation: 当前AI和HCI系统虽然被宣传为通用解决方案，但其设计仍以文本为中心，未能充分服务于主要依赖口语的语言和原住民社区。瓜拉尼语作为巴拉圭官方语言且广泛使用，却因系统设计偏向文本而受到忽视，这突显了AI语言支持与实际口语实践脱节的问题。

Method: 提出一种口语优先的多智能体架构作为标准"文本转语音"管道的替代方案。该架构将瓜拉尼语自然语言理解与专门的对话状态智能体和社区主导的治理智能体解耦，创建尊重原住民数据主权和双语现象的技术框架。重点关注话轮转换、修复和共享上下文等口语互动核心要素。

Result: 展示了一个技术框架，该框架不仅支持语言识别，更将口语对话作为一等设计需求，确保数字生态系统能够赋能而非忽视多样化的语言实践。通过解耦架构设计，实现了对原住民数据主权和双语现象的尊重。

Conclusion: 要使AI真正具有文化根基，必须从将口语语言适配到文本中心系统，转变为将口语对话视为一等设计需求。只有通过这种根本性转变，数字生态系统才能真正赋能而非忽视多样化的语言实践，确保技术发展与文化多样性相协调。
...
TL;DR: NERdME数据集包含200个手动标注的README文件，包含10,000多个标注跨度和10种实体类型，用于解决代码仓库实现级信息提取问题，填补了现有学术信息提取数据集只关注论文而忽略实现细节的空白。


<details>
  <summary>Details</summary>
Motivation: 现有学术信息提取（SIE）数据集主要关注科学论文，但忽略了代码仓库中的实现级细节。README文件描述了数据集、源代码和其他实现级工件，但其自由格式的Markdown缺乏语义结构，使得自动信息提取变得困难。需要填补这一研究空白。

Method: 引入NERdME数据集：包含200个手动标注的README文件，超过10,000个标注跨度和10种实体类型。使用大型语言模型和微调的transformer模型进行基线实验，并进行了下游实体链接实验来展示README实体的应用价值。

Result: 基线结果显示，论文级实体和实现级实体之间存在明显差异，表明将SIE基准扩展到包含README文件中可用的实体类型具有重要价值。实体链接实验证明，从README提取的实体可以支持工件发现和元数据集成。

Conclusion: NERdME数据集填补了学术信息提取领域的重要空白，通过关注代码仓库中的实现级细节，为自动提取README文件中的结构化信息提供了基准。该研究展示了将SIE基准扩展到实现级实体的价值，并证明了README实体在工件发现和元数据集成中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 患者生成文本包含患者生活经历、社会环境和护理参与等关键信息，这些因素强烈影响依从性、护理协调和健康公平。然而这些患者声音信号很少以结构化形式可用，限制了其在患者为中心的结果研究和临床质量改进中的应用。可靠提取此类信息对于大规模理解和解决健康结果的非临床驱动因素至关重要。

Method: 引入PVminer基准用于结构化提取患者声音，并提出PVminerLLM——专门为此任务设计的监督微调大型语言模型。在不同数据集和模型规模上进行评估。

Result: PVminerLLM显著优于基于提示的基线方法，在代码预测上达到83.82% F1，子代码预测80.74% F1，证据跨度提取87.03% F1。即使较小模型也能实现强性能，表明无需极端模型规模即可实现可靠的患者声音提取。

Conclusion: PVminerLLM实现了从患者生成文本中提取社会和经验信号的可扩展分析，为患者为中心的结果研究和临床质量改进提供了有效工具。代码、评估脚本和训练模型将公开发布。
...
TL;DR: 本文提出了一个用于大规模分析辅导对话的辅导行为分类法，通过混合演绎-归纳方法开发，将辅导行为分为四类，特别关注学习支持行为的连续谱系，以支持AI标注和辅导策略计算建模。


<details>
  <summary>Details</summary>
Motivation: 理解辅导有效性的关键需要系统分析辅导者在学习互动中的教学行为。现有研究缺乏支持大规模辅导对话分析的结构化框架，特别是在国家辅导观测站背景下，需要可扩展的标注工具来研究辅导行为与学习成果的关系。

Method: 采用混合演绎-归纳方法：首先综合认知科学、学习科学、课堂话语分析和智能辅导系统的研究构建初步框架；然后通过专家标注者对真实辅导转录本的迭代编码进行精炼。专家标注者具有丰富的教学和质性研究经验。

Result: 开发出包含四个类别的辅导行为分类法：辅导支持、学习支持、社会情感与动机支持、后勤支持。学习支持行为进一步按学生参与度连续谱系组织，区分引发学生推理的行为与提供直接解释或答案的行为。

Conclusion: 该分类法通过将辅导对话定义为离散的教学行为，实现了使用AI进行可扩展标注、辅导策略的计算建模，以及辅导行为与学习成果关系的实证分析，为大规模辅导研究提供了重要工具。
...
TL;DR: RouteGoT：一种预算可控的节点自适应路由框架，用于图结构推理，通过智能分配不同能力的模型到推理图的不同节点，在保持或提高准确性的同时显著减少令牌消耗。


<details>
  <summary>Details</summary>
Motivation: 现有图结构推理方法（如ToT、GoT、AGoT）虽然能提升某些基准测试的准确性，但引入了显著的令牌消耗和延迟开销，且性能增益在不同任务分布中不稳定。作者观察到图推理管道中存在阶段和节点的异质性：高质量规划和最终综合需要强大模型，而许多中间子任务可由轻量级模型准确解决。

Method: RouteGoT是一个预算可控的节点自适应路由框架，通过图内路由优先为规划和综合分配强大模型，同时根据预测的难度动态为叶子子任务分配轻量级模型和成本效益策略。框架集成了显式预算约束到全局推理调度器中，在用户指定的令牌预算下控制图扩展，实现可预测的性能-成本权衡。

Result: 在推理、检索和多跳QA基准测试中，RouteGoT匹配或提高了准确性，同时显著减少了令牌使用；相比AGoT平均实现了8.1个百分点的准确性提升和79.1%的输出令牌减少。此外，RouteGoT通过保持更优的成本-准确性权衡，优于现有路由基线，在不同预算目标和任务下表现出改进的鲁棒性。

Conclusion: RouteGoT通过识别和利用图推理管道中的异质性，智能路由不同能力的模型到不同节点，实现了更高效的成本-准确性权衡。该框架为图结构推理提供了预算可控的解决方案，在保持或提高性能的同时显著降低计算成本。
...
TL;DR: HART是一个细粒度的幻觉归因和证据检索框架，通过结构化建模解决LLM幻觉追踪问题，包含四个阶段：跨度定位、机制归因、证据检索和因果追踪。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉归因方法主要基于外部知识检索或内部模型机制，侧重于语义相似度匹配或表示级判别，难以在跨度层面建立幻觉类型、底层错误生成机制和外部事实证据之间的结构化对应关系，限制了幻觉片段的可解释性和支持/反对证据的可追溯性。

Method: 提出HART框架，将幻觉追踪形式化为包含四个阶段的结构化建模任务：1) 跨度定位，2) 机制归因，3) 证据检索，4) 因果追踪。基于此构建了首个针对幻觉追踪的结构化数据集，联合标注幻觉类型、错误机制和反事实证据集。

Result: 在所提出的数据集上的实验结果表明，HART显著优于强检索基线方法（包括BM25和DPR），验证了所提出的追踪范式在幻觉分析和证据对齐方面的有效性和泛化能力。

Conclusion: HART通过结构化建模方法解决了现有幻觉归因方法的局限性，提供了细粒度的幻觉追踪能力，增强了LLM幻觉的可解释性和证据可追溯性，为高可靠性应用领域提供了重要工具。
...
TL;DR: ReflexiCoder是一个基于强化学习的代码生成框架，通过内部化结构化推理轨迹（生成-反思-修正）到模型权重中，实现无需外部依赖的自主调试和优化能力，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统LLM代码生成的"System 1"方法在复杂算法任务上存在性能瓶颈，现有迭代优化策略依赖外部oracle、执行反馈或计算昂贵的提示-响应循环，需要更高效、自主的代码生成方法。

Method: 提出ReflexiCoder强化学习框架，采用RL-zero训练范式，通过细粒度奖励函数优化整个反思-修正轨迹，将结构化推理（初始生成、bug和优化感知的反思、自我修正）内部化到模型权重中。

Result: ReflexiCoder-8B在7个基准测试中达到SOTA：HumanEval 94.51%（Plus 87.20%）、MBPP 81.80%（Plus 78.57%）、BigCodeBench 35.00%、LiveCodeBench 52.21%、CodeForces 37.34%，推理时计算开销减少约40%。

Conclusion: ReflexiCoder实现了从外部依赖优化到内在自主反思修正的范式转变，在代码生成任务上达到或超越GPT-5.1等专有模型性能，同时显著提高token效率和推理速度。
...
TL;DR: ROSE是一种改进的LLM剪枝方法，通过重新排序剪枝顺序来提升SparseGPT框架下的剪枝效果，特别针对权重呈现列模式的情况。


<details>
  <summary>Details</summary>
Motivation: SparseGPT等基于二阶梯度（Hessian）的LLM一次性剪枝方法虽然有效，但其预定义的从左到右剪枝顺序在权重呈现列模式时会导致次优性能。本文旨在研究剪枝顺序对SparseGPT框架的影响，并提出改进方案。

Method: 提出ROSE方法：1）进行预剪枝识别候选权重；2）估计列损失和块损失；3）执行两级重新排序：基于列损失降序重新排序块内列，基于块损失重新排序块；4）引入块损失相对范围指标识别列层，实现跨整个模型的自适应重新排序。

Result: 在主流LLM（LLaMA2-7B/13B/70B、LLaMA3-8B、Mistral-7B）上的大量实验结果表明，ROSE超越了原始SparseGPT和其他对比剪枝方法。

Conclusion: ROSE通过重新排序剪枝顺序有效解决了SparseGPT在列模式权重下的性能限制，提供了一种更优的LLM剪枝方法，代码已开源。
...
TL;DR: CoCA是一个GRPO强化学习框架，通过分段信用分配联合优化置信度校准和答案准确性，实现置信度优先的范式，提升LLM的不确定性估计可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法主要是答案优先范式，在生成答案后才产生置信度，这只能衡量特定回答的正确性，限制了实际可用性。需要研究置信度优先范式，让模型在回答前输出置信度，将其解释为模型在当前策略下正确回答问题的概率。

Method: 提出CoCA框架，采用GRPO强化学习方法，通过分段信用分配联合优化置信度校准和答案准确性。该方法为置信度和答案段分配独立的奖励和组相对优势，实现稳定的联合优化并避免奖励攻击。

Result: 在数学、代码和事实问答基准测试中，CoCA显示出改进的校准和不确定性区分能力，同时保持了答案质量，从而支持更广泛的下游应用。

Conclusion: CoCA通过置信度优先范式和联合优化方法，显著提升了LLM不确定性估计的可靠性，为实际部署提供了更实用的置信度评估框架。
...
TL;DR: 本文指出传统BPE分词方法对黏着语不适用，需要开发专门的分词器来保留语素边界


<details>
  <summary>Details</summary>
Motivation: 当前主流的分词方法BPE及其变体是脚本无关且针对英语等屈折语优化的，但对于黏着语（如达罗毗荼语系、突厥语系、乌拉尔语系等）来说，单个词可能包含词根、时态、体、人称、数、性一致、格和后置词等多种语素信息，统计分词器会将这些词切分成字节对片段，破坏语素边界并增加token数量，影响语言模型性能

Method: 论文未在摘要中明确说明具体方法，但暗示需要开发专门针对黏着语的分词方法，可能涉及基于语素分析的分词策略，而不是简单的统计字节对编码

Result: 摘要未提供具体实验结果，但指出了当前BPE分词方法在黏着语处理上的根本缺陷：破坏语素边界、增加token数量，这会导致语言模型在处理这些语言时效率低下且语义理解不准确

Conclusion: 对于黏着语需要开发专门的分词方法，以保留语素边界并优化tokenization过程，这对于在这些语言上构建高效的大型语言模型至关重要
...
TL;DR: 提出了ConStory-Bench基准测试，用于评估长文本故事生成中的叙事一致性，包含2000个提示和5类19个子类型的错误分类，并开发了自动检测管道ConStory-Checker


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成长篇叙事时经常出现前后不一致的问题，如自相矛盾的事实、角色特征和世界规则，而现有的故事生成基准主要关注情节质量和流畅度，缺乏对一致性的系统评估

Method: 构建了ConStory-Bench基准测试，包含2000个提示和四个任务场景，定义了5个主要错误类别和19个细分子类型；开发了ConStory-Checker自动化管道，能够检测矛盾并为每个判断提供明确的文本证据

Result: 评估多个LLM后发现：一致性错误在事实和时间维度最常见；倾向于出现在叙事中间部分；发生在token级熵值较高的文本段中；某些错误类型倾向于共同出现

Conclusion: ConStory-Bench填补了长文本故事生成中一致性评估的空白，发现的一致性错误模式可以为改进叙事一致性提供指导，有助于未来提升长文本生成的质量
...
TL;DR: 该论文提出了一种基于LLM集成系统的联合国安理会决议语义标注方法，通过内容保存率和标签格式良好性两个评估指标来选择最佳输出，实现了高效准确的数据清洗和语义标注。
...
Method: 提出基于LLM的集成系统方法，引入内容保存率(CPR)和标签格式良好性(TWF)两个评估指标，通过多个GPT模型多次运行并选择最佳输出，构建可靠语义标注系统。

Result: GPT-4.1在两个任务中表现最佳（清洗：CPR 84.9%；语义标注：CPR 99.99%，TWF 99.92%）。GPT-4.1-mini以20%的成本实现了可比性能。集成系统成功为所有模型选择了最优输出。

Conclusion: 通过集成设计和评估指标，构建了可靠的LLM系统用于挑战性文本的语义标注，在准确性和成本效率之间取得了良好平衡。
...
TL;DR: 论文提出了一种预测用户下一步行动（NAP）的新方法，通过多模态交互数据训练LongNAP模型，结合参数化和上下文学习，在手机使用数据上取得了显著优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 真正主动的AI系统需要预测用户的下一步行动，这需要比稀疏提示更丰富的信息——需要对用户所见所做的完整上下文进行推理。当前方法缺乏对用户多模态交互历史的全面建模能力。

Method: 1) 构建大规模数据集：使用视觉语言模型标注纵向自然计算机使用数据，发布开源标注流程，标注了20名用户一个月手机使用的36万次动作；2) 提出LongNAP模型：结合参数化和上下文学习，通过策略梯度方法训练生成用户特定推理轨迹，从历史轨迹库中检索相关轨迹，并在上下文中应用这些轨迹来预测未来行动。

Result: LongNAP在留出数据上显著优于监督微调和提示基线（分别提升79%和39%），能够泛化到未见用户。17.1%的预测轨迹与用户实际行动良好对齐（LLM-judge评分≥0.5），当过滤到高置信度预测时提升至26%。

Conclusion: 从用户行为的完整上下文中学习以预测用户需求现在是一个可行的任务，具有巨大的机会。LongNAP展示了通过结合参数化和上下文学习来建模长期交互历史的有效性。
...
TL;DR: 该研究探讨了在大型语言模型（8B Llama）中解决生态谬误问题，通过建模作者的语言上下文（HuLM任务）来提升模型性能。实验表明，在微调阶段考虑作者上下文（HuFT）能改善模型表现，而基于QLoRA的HuLM预训练能产生可泛化的人类感知模型。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型训练和推理忽视了一个基本语言事实：同一作者生成的多个文本序列之间存在依赖关系。先前研究表明，解决这种生态谬误形式能显著提升较小规模（约124M）GPT模型的性能。本研究旨在探索在更大规模模型（8B Llama）中，通过建模作者语言上下文（HuLM任务）来解决生态谬误是否能带来类似益处。

Method: 研究探索了处理作者语言在其时间顺序文本上下文中的变体方法。具体包括：1）使用HuLM目标进行预训练，建模作者上下文；2）在微调阶段使用作者上下文（HuFT：人类感知微调）；3）采用QLoRA技术进行HuLM预训练和微调。

Result: 实证比较表明：1）仅通过QLoRA在微调阶段解决生态谬误，就能提升8B模型相对于标准微调的性能；2）基于QLoRA的HuLM预训练能产生人类感知模型，该模型通过线性任务分类器训练即可在八个下游任务上获得改进性能。

Conclusion: 研究结果表明，在语言原始生成者（作者）的上下文中建模语言具有实用性和重要性。解决生态谬误不仅能提升大型语言模型性能，还能产生可泛化的人类感知模型，为语言建模提供了新的研究方向。
...
TL;DR: 提出结构化风格重写框架，通过显式解耦词汇、句法和语用三个风格维度，结合CoT蒸馏隐式风格条件策略，使小模型在角色扮演中超越大模型的表现


<details>
  <summary>Details</summary>
Motivation: 小语言模型在高度风格化的角色扮演中面临数据稀缺和风格解耦复杂性的挑战，标准监督微调只能捕捉表层语义而无法复现角色的句法和语用细微差别，导致"角色出戏"生成

Method: 提出结构化风格重写框架：1) 将风格显式解耦为三个可解释维度：词汇特征（通过PMI）、句法模式（基于PCFG规则）和语用风格；2) 引入通过思维链蒸馏的隐式风格条件策略，利用训练期间的显式推理轨迹作为强归纳偏置，使模型的潜在表示与结构化风格特征对齐

Result: 在特定高风格化领域（动漫角色）的广泛实验中，该方法使Qwen-1.7B模型在风格一致性和语义保真度方面显著超越更大的基线模型（如4B标准SFT）

Conclusion: 该方法为在消费级硬件上进行推理和部署提供了数据高效的范式，实现了小模型的高保真风格化生成，无需在推理时使用显式推理标记
...
TL;DR: 该研究开发了可解释的心理健康预测模型，整合个人心理特质与情境特征，在社交媒体数据中识别适应性/非适应性自我状态，理论驱动特征在保持竞争力的同时提供更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 心理健康不是固定特质而是动态过程，受个体倾向与情境背景相互作用影响。传统方法缺乏对心理状态动态性和情境敏感性的考量，需要开发既能预测又能解释心理健康变化的计算模型。

Method: 基于互动主义和建构主义心理学理论，整合个人层面心理特质（韧性、认知扭曲、内隐动机）与情境8 DIAMONDS框架推导的语言推断情境特征，并与心理测量学语言模型嵌入进行比较。

Result: 理论驱动特征在预测幸福感方面具有竞争力表现，同时提供更好的可解释性。定性分析进一步显示最预测幸福感的特征具有心理一致性，验证了理论框架的有效性。

Conclusion: 计算建模与心理学理论整合对于在情境敏感且人类可理解的方式下评估动态心理状态具有重要价值，为心理健康研究提供了理论驱动、可解释的分析框架。
...
TL;DR: Track-SQL框架通过双提取模块增强生成式语言模型，提升多轮Text-to-SQL性能
...
TL;DR: MASFactory是一个基于图的框架，用于编排基于LLM的多智能体系统，通过Vibe Graphing将自然语言意图编译为可编辑的工作流规范，提供可重用组件和可视化工具。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的多智能体系统在实现复杂图工作流时需要大量手动工作，复用性有限，且难以集成异构的外部上下文源。
...
Conclusion: MASFactory通过图中心框架解决了LLM-based多智能体系统工作流编排的挑战，提供了更高效、可重用且易于集成的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多视图空间推理方面表现不佳，即使有多个视角可用，模型也常常未能充分利用跨视图关系，而是依赖单图像捷径，导致在视角变换和遮挡敏感案例中表现脆弱。

Method: ViewFusion采用两阶段框架：第一阶段进行空间预思考，推断跨视图的视角关系和空间变换，形成超越简单重描述的中间工作空间；第二阶段基于该工作空间进行问题驱动的推理以生成最终预测。训练过程包括合成推理监督和基于GRPO的强化学习。

Result: 在MMSI-Bench基准测试中，ViewFusion相比Qwen3-VL-4B-Instruct提升了5.3%的准确率，在需要真正跨视图对齐的示例上取得了最大增益。

Conclusion: 通过显式分离跨视图空间预对齐和问题回答，ViewFusion能够更有效地进行多视图空间推理，特别是在需要跨视图对齐的任务上表现出显著改进。
...
TL;DR: 该研究评估了四种开源大语言模型在奥地利德语A级考试作文自动评分中的应用，发现模型在评分细则子维度上与人类评分者的一致性最高仅为40.6%，最终成绩匹配率仅为32.8%，表明当前模型尚无法在实际评分环境中使用。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分系统旨在减轻教师评分工作量并减少主观偏见。虽然早期系统依赖手工特征和统计模型，但大语言模型的发展为评估学生写作提供了前所未有的灵活性。本研究旨在探索最新开源大语言模型在奥地利德语A级考试作文评分中的应用，特别关注基于评分细则的评估。

Method: 研究处理了101份匿名学生考试作文，涵盖三种文本类型。评估了四种大语言模型：DeepSeek-R1 32b、Qwen3 30b、Mixtral 8x7b和LLama3.3 70b，采用不同上下文和提示策略。研究重点关注基于评分细则的评估方法。

Result: 大语言模型在评分细则提供的子维度上与人类评分者的最大一致性为40.6%，最终成绩与人类专家给出的成绩匹配率仅为32.8%。结果表明，尽管较小模型能够使用标准化评分细则进行德语作文评分，但准确性不足以在实际评分环境中使用。

Conclusion: 当前的开源大语言模型在奥地利德语A级考试作文评分中表现有限，无法达到实际应用所需的准确性水平。模型在评分细则子维度和最终成绩方面与人类专家的一致性都较低，表明自动作文评分系统仍需进一步改进才能在实际教育环境中可靠使用。
...
TL;DR: 该研究通过持续预训练让大语言模型接触特定领域文本，模拟经验积累，发现模型能力呈双峰分布，存在"表达型通才"和"抑制型专才"两种最优类型，并揭示了"抑制优势"现象。


<details>
  <summary>Details</summary>
Motivation: 人类问题解决受益于多样化的风格和人格特质，但当前大语言模型的发展主要关注统一性能基准，偏向特定的行为倾向如自信性。研究旨在探索多样化经验如何塑造机器个性并影响问题解决能力。

Method: 采用持续预训练方法，以无监督方式让模型接触领域特定文本，模拟经验积累过程。通过机器人格量表（MPI）基于大五人格框架量化模型变体的人格特质，分析其与语言风格和推理行为的关系。

Result: 模型能力呈双峰分布，在"表达型通才"和"抑制型专才"处达到峰值。发现"抑制优势"现象，即降低社交特质能提升复杂推理性能。建立了训练数据语言学特征（如祈使句频率）与词汇多样性之间的因果关系。

Conclusion: 该研究为"人格工程学"提供了路线图，表明通过控制训练数据的语言学特征可以系统性地塑造机器个性，从而优化特定任务的问题解决能力。
...
TL;DR: 本文提出了一种将文本中的省略三段论转化为逻辑论证的集成管道，结合LLM生成隐含前提、翻译为逻辑公式，以及基于SAT求解器的神经符号推理器来确定蕴涵关系。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本和对话论证通常是省略三段论（部分前提和/或主张是隐含的）。现有的NLP方法只能识别省略三段论但无法解码其底层逻辑，而基于逻辑的方法则假设存在足够的知识库公式来进行溯因解码。目前缺乏将省略三段论的文本组件转化为逻辑论证并生成解码所需逻辑公式的系统方法。

Method: 提出一个集成管道，包含三个组件：1）使用大型语言模型基于显式前提和主张生成中间隐含前提；2）使用另一个LLM将自然语言翻译为逻辑公式；3）基于SAT求解器的神经符号推理器来确定蕴涵关系。

Result: 在两个省略三段论数据集上评估了该管道，在正确选择隐含前提方面表现出有希望的性能，通过精确率、召回率、F1分数和准确率进行衡量。

Conclusion: 该研究填补了将文本省略三段论转化为逻辑论证的系统方法空白，通过集成LLM和神经符号推理器，实现了从自然语言到逻辑公式的转换和蕴涵关系的确定。
...
TL;DR: 该论文提出了一种基于图的框架，用于检测、分析和分类对立叙事及其底层实体，通过将叙事表示为实体交互图，并引入节点级因果估计来获得最小因果子图，从而在分类任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前文本分析方法依赖于预定义本体论标注的数据，往往将人类偏见嵌入黑盒模型中。尽管这些方法取得了接近完美的性能，但它们利用的是非结构化、线性的模式识别，而非建模话语中自然出现的实体间的结构化交互。

Method: 提出基于图的框架：1) 将叙事表示为实体交互图；2) 在节点层面引入因果估计，将构建的句子图提炼为最小因果子图，从而获得每个贡献对最终分类的因果表示；3) 基于此表示构建分类流程。

Result: 该方法在对立思维分类任务上超越了现有方法。

Conclusion: 通过将叙事建模为实体交互图并结合因果分析，该方法能够更好地捕捉话语中的结构化交互关系，减少人类偏见的影响，并在分类性能上优于传统基于预定义本体论的方法。
...
TL;DR: CRIMSON是一个临床基础的胸片报告生成评估框架，通过诊断正确性、上下文相关性和患者安全三个维度评估报告质量，采用临床显著性分级和错误分类法，与放射科医生评估高度一致。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标缺乏临床上下文考虑，对正常或临床不显著发现给予不当权重，无法准确反映报告的实际临床价值。需要开发一个能结合完整临床背景、基于指南决策规则、并能区分临床重要性的评估框架。

Method: CRIMSON框架包含：1）整合患者年龄、指征和指南决策规则的完整临床上下文；2）将错误分类为假阳性发现、遗漏发现和八种属性级错误；3）基于与心胸放射科医生合作制定的指南，将发现分为紧急、可行动非紧急、不可行动、预期/良性四个临床显著性级别；4）采用严重性感知加权，优先考虑临床重要错误。

Result: 在ReXVal验证中，CRIMSON与六名认证放射科医生标注的临床显著错误计数高度一致（Kendall's tau = 0.61-0.71；Pearson's r = 0.71-0.84）。在RadJudge和RadPref两个新基准测试中，CRIMSON与专家判断和放射科医生偏好保持最强一致性。发布了评估基准、指标和微调的MedGemma模型。

Conclusion: CRIMSON提供了一个临床基础的胸片报告生成评估框架，通过整合完整临床上下文、错误分类和严重性感知加权，能够准确反映报告的临床价值，与放射科医生评估高度一致，为可重复评估提供了标准化工具。
...
<details>
  <summary>Details</summary>
Motivation: 主观多轮对话任务（如情感支持）需要适应动态用户状态并优化长期交互质量的对话策略。然而，强化学习在此类场景面临挑战：仅基于结果的训练将多轮信用分配压缩为单一轨迹级奖励，而简单的轮级分组采样在交互环境中成本过高。缺乏可靠的过程监督限制了RL在主观对话中的应用。

Method: 提出MAPO算法：1）利用法官模型提供密集过程反馈，通过蒙特卡洛回报传播长时程效应；2）引入混合优势估计器，结合轮级归一化和批次级归一化，实现细粒度且可扩展的信用分配；3）无critic架构，避免价值函数估计误差；4）在多个主观对话基准（EMPA、EmoBench、EQ-Bench）和不同模型规模（7B-32B）上验证。

Result: 在多个基准上一致提升训练稳定性和最终性能：1）EMPA上提升率最高达9个百分点，对话分数比7B基础模型提高+43.2；2）仅在EMPA风格环境训练，但能很好泛化到未见的情感智能基准，EmoBench提升最高+4分，EQ-Bench提升+3.5分；3）优于仅基于结果的GRPO和单级归一化基线；4）在不同模型规模（7B-32B）上均有效。

Conclusion: 密集过程监督与混合级归一化相结合，能够实现有效且可扩展的强化学习，适用于主观、开放式的多轮对话任务。该方法通过细粒度信用分配和长时程效应传播，解决了主观对话中RL训练的关键挑战，为情感支持等复杂对话系统的优化提供了新途径。
...
Method: 提出AI-CROWD协议，利用大语言模型集合的集体输出近似真实标注；通过多数投票聚合输出，并使用诊断指标分析一致/分歧模式，识别高置信度分类并标记潜在模糊性或模型特定偏差

Result: 协议生成基于共识的近似标注，而非声称是真实标注；能够识别高置信度分类，同时标记潜在模糊区域和模型偏差

Conclusion: AI-CROWD协议为大规模内容分析提供了一种实用的解决方案，通过LLM集合的集体智慧来近似真实标注，克服了传统人工标注的局限性
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG生成器基准测试覆盖能力有限，无法在统一条件下同时评估多个实际应用所需的关键能力（如长上下文整合、多步推理、表格解释、证据缺失时弃权等），需要更全面的评估框架。

Method: 构建包含114个人工构造的日语问题（含机器翻译的英文版本）的数据集，使用虚构实体和场景确保答案基于外部文档。定义五个评估类别：集成、推理、逻辑、表格、弃权，每个类别进一步细分为实际评估方面。采用LLM-as-a-Judge进行评分，报告类别和总体准确率。

Result: 在API和开源模型测试中，没有模型总体准确率超过90%。基准测试能够清晰测量各模型在不同类别中的优势和弱点，为实际RAG部署中的模型选择和RAG专用模型构建提供有价值的度量标准。

Conclusion: LIT-RAGBench填补了现有评估与实际应用之间的差距，通过系统覆盖多类别组合模式，成为RAG生成器评估的重要工具。数据集和评估代码已开源发布。
...
TL;DR: FlashPrefill提出了一种通过瞬时模式发现和阈值处理的超快速预填充框架，解决了长上下文建模中注意力机制二次复杂度的瓶颈问题，在256K序列上实现27.78倍加速。


<details>
  <summary>Details</summary>
Motivation: 长上下文建模是大语言模型的关键能力，但注意力的二次复杂度在计算密集的预填充阶段成为关键瓶颈。现有的稀疏注意力机制要么有显著的搜索延迟，要么稀疏性不足。

Method: FlashPrefill采用快速块搜索技术同时定位动态垂直、斜线和块稀疏注意力模式，并引入动态阈值机制，避免排序或累积注意力分数的过高开销，有效消除长尾分布以增强稀疏性。

Result: 在256K序列上实现27.78倍加速，在4K上下文长度上仍保持1.71倍加速，表明该方法在不同序列尺度上都具有鲁棒性和实用价值。

Conclusion: FlashPrefill通过创新的模式发现和阈值处理机制，显著提升了长上下文建模的预填充效率，解决了现有稀疏注意力方法的局限性，为大规模语言模型的实际应用提供了重要技术突破。
...
<details>
  <summary>Details</summary>
Motivation: 显式思维链推理虽然能提升大语言模型的推理性能，但会产生高推理成本（冗长的token级追踪）。现有方法主要通过截断或剪枝来减少开销，但只是截断模型输出而非内化模型思考。潜在推理方法面临两个关键挑战：1）刚性点对点对齐无法捕捉整个推理段的密集、变长语义；2）缺乏可解释性，潜在状态难以解码或审计。

Method: SPOT框架包含两个核心技术：1）跨段语义对齐（Span-level Semantic Alignment），使用Sinkhorn最优传输目标，将每个暂停标记软匹配到整个推理段的语义；2）冻结头部解码约束（Frozen-Head Decoding Constraint），保持潜在状态在冻结的预训练LM头部下可直接解码为token分布，实现潜在思考的可读关键词解释。

Result: 在推理基准测试中，SPOT平均提升准确率2.3个百分点，同时减少生成token数量37.5%，并能提供潜在推理过程的忠实语义解释。

Conclusion: SPOT通过灵活的潜在推理框架，在减少推理开销的同时提升性能，并通过可解码的潜在状态提供更好的可解释性，为高效且可解释的推理系统提供了新方向。
...
TL;DR: 该研究对GPT-4o-Mini、Gemini-2.5-Flash、Llama 3.2、Mistral和Gemma 3等当代大语言模型在印度、东亚和东南亚地区的文化对齐性进行了多语言审计，发现这些模型在宗教等敏感领域存在严重对齐问题，特别是对少数群体的代表性不足和负面刻板印象放大。


<details>
  <summary>Details</summary>
Motivation: 大语言模型越来越多地在多语言、多文化环境中部署，但其主要依赖英语中心训练数据，可能导致与不同社会的多元文化价值观不匹配。研究者旨在通过系统审计揭示LLMs在敏感文化领域（特别是宗教）的对齐问题，为全球公平部署提供依据。

Method: 采用多语言、多文化审计方法，聚焦印度、东亚和东南亚地区。使用内部表征分析（log-probs/logits）比较模型观点分布与真实公众态度。进行多维度分析，包括轻量级干预（人口统计提示和母语提示）效果评估，并在偏见基准测试（CrowS-Pairs、IndiBias、ThaiCLI、KoBBQ）上进行下游评估。

Result: 主流模型在广泛社会议题上能与公众意见对齐，但在宗教观点上持续失败，特别是对少数群体的代表性不足，经常放大负面刻板印象。轻量级干预措施（人口统计提示和母语提示）能部分缓解但无法消除文化差距。下游偏见基准测试显示在敏感情境下存在持续伤害和代表性不足问题。

Conclusion: 研究强调需要系统性、基于地区的审计来确保大语言模型的全球公平部署。当前模型在文化对齐方面存在显著缺陷，特别是在宗教等敏感领域，需要更深入的文化理解和本地化训练来解决这些问题。
...
TL;DR: 研究探索ChatGPT模仿原创艺术作品生成新图像的人工视觉创造力，发现AI模仿作品在色彩纹理相似度与构图概念感知相似度之间存在显著差距，建议使用"风格转换仪表板"的多指标评估而非单一风格指标。


<details>
  <summary>Details</summary>
Motivation: 探索人工智能在视觉艺术创作中的创造力，特别是ChatGPT模仿原创艺术作品生成新图像的能力，评估AI生成作品与原始作品之间的相似性和艺术价值。

Method: 邀请12位来自罗马尼亚、保加利亚、法国、奥地利和英国的艺术家，每人提供三件原创艺术作品；使用ChatGPT生成模仿这些作品的图像；结合人类评估（艺术家评分和评论）与计算方法（检测视觉和风格相似性或差异）进行分析。

Result: 研究发现色彩和纹理相似度与构图、概念和感知相似度之间存在显著差距；艺术家认为ChatGPT的模仿作品缺乏维度感、背景和意图感，更像是释义或近似引用，而非能引发情感的有价值艺术作品。

Conclusion: 建议使用包含互补指标的"风格转换仪表板"来评估模仿作品与原始作品的相似性，而非依赖单一风格指标；AI在艺术模仿方面仍存在局限性，特别是在捕捉作品的深层概念和情感维度方面。
...
TL;DR: 该研究将数学问题理解建模为数学实体关系抽取任务，使用Transformer模型自动提取数学文本中的操作数和运算符关系，BERT模型达到99.39%准确率，并引入SHAP可解释性分析增强模型透明度。


<details>
  <summary>Details</summary>
Motivation: 数学文本理解具有挑战性，因为存在专门的数学实体和复杂的相互关系。需要一种系统方法来理解数学问题中的结构和语义关系，以支持自动化问题求解、知识图谱构建和智能教育系统。

Method: 将数学问题解释建模为数学实体关系抽取任务，将操作数视为实体，运算符视为关系。应用基于Transformer的模型自动提取这些关系，使用BERT模型进行关系抽取，并引入SHAP可解释人工智能技术分析模型预测。

Result: BERT模型在数学实体关系抽取任务中表现最佳，达到99.39%的准确率。可解释性分析揭示了特定文本和数学特征如何影响关系预测，提供了特征重要性和模型行为的深入见解。

Conclusion: 通过结合基于Transformer的学习、任务特定数据集和可解释建模，该工作为数学实体关系抽取提供了一个有效且可解释的框架，支持未来在自动化问题求解、知识图谱构建和智能教育系统中的应用。
...
TL;DR: 该研究通过创建包含道义模态的Wason选择任务数据集，系统比较了LLMs在道义规则和描述性规则下的条件推理能力，发现LLMs与人类类似，在道义规则下表现更好，并表现出匹配偏误的错误模式。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在语言能力上的进步，其推理能力受到越来越多的关注。人类推理通常在特定领域表现良好，特别是在规范性而非纯形式化语境中。尽管先前研究比较了LLM和人类推理，但LLM推理的领域特异性仍未得到充分探索。本研究旨在探究LLMs在不同规则类型下的条件推理表现，特别是道义规则与描述性规则的区别。

Method: 研究引入了一个新的Wason选择任务数据集，该数据集明确编码了道义模态，以系统区分道义条件句和描述性条件句。使用该数据集检验LLMs在道义规则下的条件推理能力。进一步分析观察到的错误模式是否更好地由确认偏误（倾向于寻找支持规则的证据）或匹配偏误（倾向于忽略否定并选择与规则元素词汇匹配的项目）来解释。

Result: 结果显示，与人类类似，LLMs在道义规则下推理表现更好，并表现出类似匹配偏误的错误模式。这些发现表明LLMs的性能在不同规则类型间存在系统性差异，其错误模式可以与该范式中已知的人类偏误相平行。

Conclusion: LLMs的条件推理能力在不同规则类型下表现出系统性差异，特别是在道义规则下表现更优，且错误模式与人类认知偏误（特别是匹配偏误）相似。这为理解LLMs的推理机制及其与人类认知的相似性提供了重要见解。
...
TL;DR: 该研究对大型语言模型在英语作为第二语言写作自动评分中的四种主要方法进行了首次统一比较，发现结合k-SFT和RAG的方法在雅思写作任务2上达到93%的F1分数，揭示了不同方法在准确性、成本和鲁棒性之间的权衡。
...
Method: 在统一的雅思写作任务2基准上评估了四种LLM-based AES方法：(1)基于编码器的分类微调；(2)零样本和少样本提示；(3)指令微调和检索增强生成；(4)监督微调结合直接偏好优化和RAG。

Result: 研究揭示了不同方法在准确性、成本和鲁棒性之间的明确权衡。最佳配置（结合k-SFT和RAG）在雅思写作任务2上达到了最强的整体结果，F1分数为93%。

Conclusion: 该研究首次对现代LLM-based AES策略在英语作为第二语言写作中进行了统一的实证比较，展示了这些方法在自动评分写作任务中的潜力，为实际应用提供了重要参考。
...
TL;DR: 该论文通过将三段论数据集转换为适合溯因推理的形式，研究最先进的大语言模型在溯因推理中的准确性和偏见，探讨其与人类认知的差异。


<details>
  <summary>Details</summary>
Motivation: 当前研究关注大语言模型与人类推理能力的比较，特别是两者都表现出相似的偏见，如拒绝与常识相悖的逻辑有效推论。然而，批评大语言模型的这些偏见可能不公平，因为人类推理不仅涉及形式演绎，还包括从有限信息中得出试探性结论的溯因推理。需要研究大语言模型在溯因推理中的表现，以更全面地评估其推理能力。

Method: 将三段论数据集转换为适合溯因推理的形式，用于评估最先进的大语言模型在溯因推理任务中的准确性。通过这种转换，研究模型是否在溯因推理中表现出偏见，并识别潜在的改进领域。

Result: 论文未提供具体实验结果，但研究旨在揭示大语言模型在溯因推理中的表现特征，包括是否存在偏见以及需要改进的方向。

Conclusion: 该研究对于推进大语言模型在复杂推理任务中的理解和应用至关重要，为弥合机器与人类认知之间的差距提供了见解，强调了超越形式演绎的语境化推理的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法大多采用"一刀切"范式，忽视了用户在专业知识、目标和认知需求方面的差异。虽然大语言模型可以将技术解释转化为自然语言，但存在忠实性和幻觉问题。

Method: 提出PONTE框架，将个性化建模为闭环验证和适应过程而非提示工程。包含三个核心组件：(1)低维偏好模型捕捉风格需求；(2)基于结构化XAI工件的偏好条件生成器；(3)验证模块确保数值忠实性、信息完整性和风格对齐，可选支持检索增强论证。用户反馈迭代更新偏好状态，实现快速个性化。

Result: 在医疗和金融领域的自动和人工评估表明，验证-精炼循环相比无验证生成显著提高了完整性和风格对齐。人工研究进一步证实：预期偏好向量与感知风格高度一致、对生成随机性具有鲁棒性、获得一致积极的质量评估。

Conclusion: PONTE通过将个性化建模为闭环验证和适应过程，解决了可解释AI中的个性化需求和可靠性问题，在多个领域验证了其有效性，为构建自适应且可靠的可解释AI叙事提供了可行框架。
...
TL;DR: 本文提出了一种支持多语言和口音的上下文感知ASR框架，通过轻量级投影模块结合冻结语音编码器和仅解码器语言模型，使用对比学习对齐语音与上下文表示，在11种语言和5种英语方言的1500多小时真实对话语音上实现超过5%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统大多局限于单语言设置和短孤立话语，而现有的上下文感知ASR存在两个关键挑战：有限的多语言支持和缺乏语音与上下文表示之间的原则性对齐。

Method: 提出上下文感知多语言ASR框架，结合冻结语音编码器和仅解码器语言模型，通过轻量级投影模块连接，支持对话历史和偏置词等结构化上下文提示，并采用对比学习目标在共享嵌入空间中对齐语音与上下文表示。

Result: 在11种语言和5种英语方言的1500多小时真实世界对话语音评估中，上下文输入持续改善识别质量，对比对齐在不同上下文类型上提供额外增益，总体性能提升超过5%。

Conclusion: 研究结果表明上下文建模和跨模态对齐在多语言ASR中的重要性，提出的框架在保持预训练模型模块性的同时，有效支持多语言和口音识别。
...
TL;DR: KCLarity团队参与SemEval 2026的CLARITY任务，研究政治话语中歧义和回避技术的分类，比较两种建模方法并探索辅助训练变体和零样本解码器模型。


<details>
  <summary>Details</summary>
Motivation: 参与SemEval 2026的CLARITY共享任务，该任务专注于政治话语中歧义和回避技术的分类，旨在开发有效的模型来识别和分析政治沟通中的不清晰表达策略。

Method: 研究两种建模方法：(1)直接预测清晰度标签；(2)先预测回避标签，然后通过任务分类层次结构推导清晰度。探索多种辅助训练变体，并在回避优先的框架下评估解码器模型的零样本性能。

Result: 两种建模方法表现相当。在编码器模型中，RoBERTa-large在公开测试集上取得最佳结果；而零样本GPT-5.2在隐藏评估集上表现出更好的泛化能力。

Conclusion: 两种建模方法效果相似，但不同模型在不同数据集上各有优势：RoBERTa-large在公开测试集表现最佳，而GPT-5.2在隐藏评估集上泛化能力更强，表明模型选择应考虑具体应用场景。
...
TL;DR: RoboLayout扩展了LayoutVLM，通过集成显式可达性约束和局部优化策略，生成适合不同具身智能体交互的室内场景布局。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在生成语义连贯的3D场景布局方面取得进展，但生成的布局往往缺乏对具身智能体交互可行性的考虑，特别是在物理受限的室内环境中。

Method: 1. 在可微分布局优化过程中集成显式可达性约束；2. 引入局部细化阶段，选择性重新优化有问题的物体放置；3. 支持多种具身智能体抽象（服务机器人、仓库机器人、不同年龄段人类、动物等）。

Result: RoboLayout在保持LayoutVLM语义对齐和物理合理性的同时，增强了面向具身智能体的室内场景生成适用性，在不同场景配置下均表现出良好性能。

Conclusion: RoboLayout通过集成智能体感知推理和改进的优化稳定性，成功解决了具身智能体在物理约束室内环境中的交互可行性问题，为智能体中心化的室内场景生成提供了有效解决方案。
...
TL;DR: 论文提出CoT-Control评估套件来衡量模型控制思维链内容的能力，发现当前模型对思维链的控制能力远低于对最终输出的控制能力，且这种能力受模型规模、RL训练、计算资源和问题难度等因素影响。


<details>
  <summary>Details</summary>
Motivation: 思维链监控是检测现代推理模型不当行为和理解其动机的有前景工具，但如果模型能够控制其在思维链中表达的内容，可能会破坏思维链的可监控性。需要测量这种不良能力——思维链可控性。

Method: 引入CoT-Control评估套件，包含要求模型在遵循特定思维链指令的同时解决问题的任务（例如，在不使用"染色体"一词的情况下推理遗传学问题）。评估不同模型在直接请求、激励、监控告知和对抗性提示优化等条件下的表现。

Result: 推理模型的思维链可控性显著低于输出可控性（Claude Sonnet 4.5的思维链可控性仅为2.7%，而输出可控性为61.9%）。可控性随模型规模增大而提高，但随更多RL训练、测试时计算资源增加和问题难度提高而降低。即使在激励逃避监控的情况下，可控性失败仍然存在，尽管告知被监控时可控性略有提高。对抗性提示优化未能显著提升可控性。

Conclusion: 目前思维链可控性不太可能成为思维链可监控性的失效模式，但低可控性的机制尚不清楚。鉴于其对维护思维链可监控性的重要性，建议前沿实验室在未来模型中跟踪思维链可控性。
...
TL;DR: ProEvolve是一个基于图的框架，用于以可编程方式演化智能体环境，通过图变换统一表示和传播数据、工具和模式的更新，以评估智能体对动态环境的适应性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设静态环境，忽略了真实世界的演化特性以及智能体对环境变化的鲁棒性。需要一种可扩展且可控的环境演化方法来更好地评估智能体对现实世界动态的适应能力。

Method: 提出ProEvolve框架，核心是类型化关系图，统一显式表示环境的数据、工具和模式。通过图变换表达能力添加、移除或修改，并在工具、模式和数据访问之间一致传播更新。支持：(1) 将演化动态编程为图变换以自动生成环境；(2) 通过子图采样和编程实例化任务沙箱。

Result: 通过将单个环境演化为200个环境和3,000个任务沙箱来验证ProEvolve，并相应地对代表性智能体进行基准测试。

Conclusion: ProEvolve提供了一个可编程的图基框架，能够以可扩展和可控的方式演化智能体环境，从而更好地评估智能体对现实世界动态的适应性。
...
### [174] [An Interactive Multi-Agent System for Evaluation of New Product Concepts](https://arxiv.org/abs/2603.05980)
*Bin Xuan,Ruo Ai,Hakyeon Lee*

Main category: cs.AI

TL;DR: 本研究提出基于大语言模型的多智能体系统来自动化产品概念评估，通过八个专业领域智能体使用RAG和实时搜索工具收集客观证据，在技术和市场可行性维度上进行结构化审议，验证了与行业专家评估结果的一致性。


<details>
  <summary>Details</summary>
Motivation: 传统专家主导的产品概念评估方法存在主观偏见、时间和成本高等局限性，需要更客观、高效的自动化评估方法来支持企业的战略资源分配和项目成功决策。

Method: 1. 建立技术和市场可行性两个主要评估维度；2. 构建包含八个专业领域虚拟智能体（如研发、营销等）的多智能体系统；3. 使用检索增强生成和实时搜索工具收集客观证据；4. 基于专业产品评审数据对智能体进行微调；5. 通过结构化审议流程验证产品概念；6. 以专业显示器概念为案例进行验证。

Result: 案例研究表明，系统对专业显示器概念的评估排名与资深行业专家的评估结果一致，验证了多智能体评估方法的可用性和有效性。
...
### [175] [Agentic LLM Planning via Step-Wise PDDL Simulation: An Empirical Characterisation](https://arxiv.org/abs/2603.06064)
*Kai Göbel,Pierrick Lorang,Patrik Zips,Tobias Glück*

Main category: cs.AI

TL;DR: PyPDDLEngine是一个开源的PDDL仿真引擎，通过MCP接口将规划操作暴露为LLM工具调用，让LLM作为交互式搜索策略进行任务规划。在IPC Blocksworld任务上评估显示，agentic LLM规划比直接LLM规划有3%的优势，但成本高5.7倍，且两种LLM方法都产生比经典规划器更短的规划。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型能否作为可行的规划器与经典符号方法竞争，研究在任务规划问题中LLM的表现，特别是通过交互式搜索策略的方式。

Method: 开发PyPDDLEngine开源PDDL仿真引擎，通过Model Context Protocol接口将规划操作暴露为LLM工具调用。LLM作为交互式搜索策略，一次选择一个动作，观察结果状态，并可重置重试。在102个IPC Blocksworld实例上评估四种方法：Fast Downward lama-first和seq-sat-lama-2011作为经典基线，直接LLM规划，以及通过PyPDDLEngine的agentic LLM规划。

Result: Fast Downward达到85.3%成功率。直接LLM规划成功率为63.7%，agentic LLM规划为66.7%，后者有3%的优势但每个解决方案的token成本高5.7倍。在大多数共同解决的难度块中，两种LLM方法都产生比seq-sat-lama-2011更短的规划，尽管后者有迭代质量改进。

Conclusion: agentic增益取决于环境反馈的性质。编码智能体受益于编译器错误和测试失败等外部基础信号，而PDDL步骤反馈是自我评估的，智能体在没有外部验证的情况下评估自己的进展。LLM规划结果与训练数据回忆一致，而非可泛化的规划能力。
...
<details>
  <summary>Details</summary>
Motivation: 在定量双极论证框架中，现有的渐进语义通常对称处理攻击和支持关系，但实际场景中攻击者和支持者可能扮演不对称角色。需要一种更灵活、可解释的语义来更好地建模这种不对称性。

Method: 提出聚合语义家族，采用三阶段计算：1) 计算攻击者的全局权重；2) 计算支持者的全局权重；3) 将这两个值与论证的内在权重进行聚合。讨论了三个聚合函数应满足的性质及其与经典渐进语义原则的关系。

Result: 通过简单示例和最终示例（测试比较500种聚合语义）展示了聚合语义的可能行为范围。三阶段分解使计算更具参数化，保持了双极性，并产生了更易理解的渐进语义。

Conclusion: 聚合语义为定量双极论证框架提供了一种新颖、灵活且可解释的渐进语义方法，通过分开处理攻击者和支持者，更好地建模了实际论证场景中的不对称关系。
...
TL;DR: CliqueFlowmer：一种基于离线模型优化（MBO）的计算材料发现新方法，通过结合团簇优化、Transformer和流生成技术，在材料优化任务中显著优于传统生成模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的生成模型在计算材料发现中存在局限性，由于其最大似然训练难以大胆探索材料空间中有吸引力的区域，需要一种能够直接优化目标材料属性的新方法。

Method: 提出CliqueFlowmer模型，将团簇优化（clique-based MBO）的最新进展与Transformer和流生成技术相结合，实现离线模型优化方法，将目标材料属性的直接优化融入生成过程。

Result: CliqueFlowmer在材料优化能力上表现出色，其生成的材料在性能上显著优于基于生成模型的基线方法。

Conclusion: CliqueFlowmer为计算材料发现提供了一种有效的替代方案，通过开源代码支持专业材料优化问题和跨学科研究。
...
<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和洪水，导致城市交通系统中断增加。制定有效适应策略面临挑战：基础设施投资具有长期性、序列性；气候不确定性大；洪水、基础设施和交通影响之间存在复杂相互作用。

Method: 提出基于强化学习(RL)的决策支持框架，构建为综合评估模型(IAM)。整合降雨预测与洪水建模、交通模拟、基础设施和交通直接间接影响量化。RL方法学习平衡投资维护成本与避免影响的适应策略。

Result: 在哥本哈根市中心2024-2100年案例研究中，测试多种适应方案及不同信念和实际气候情景。框架优于传统优化方法：发现协调的空间和时间适应路径；学习影响减少与适应投资之间的权衡；产生更具韧性的策略。

Conclusion: 强化学习作为灵活决策支持工具，在气候不确定性下进行适应性基础设施规划具有巨大潜力。框架能够处理长期序列投资、深度不确定性和复杂系统相互作用，为城市洪水适应提供有效解决方案。
...
TL;DR: EpisTwin是一个神经符号框架，通过构建可验证的个人知识图谱来解决个人AI中数据碎片化问题，结合图检索增强生成和在线深度视觉细化实现复杂推理。


<details>
  <summary>Details</summary>
Motivation: 个人AI面临用户数据分散在孤立数据孤岛中的问题，现有检索增强生成方法依赖非结构化向量相似性，无法捕捉潜在语义拓扑和时间依赖关系，限制了整体意义构建能力。

Method: 1. 使用多模态语言模型将异构跨应用数据提升为语义三元组，构建用户中心化的个人知识图谱；2. 推理时通过智能协调器结合图检索增强生成与在线深度视觉细化，在原始视觉上下文中动态重新锚定符号实体。

Result: 开发了PersonalQA-71-100合成基准来模拟真实用户数字足迹并评估EpisTwin性能，在一系列最先进的评判模型上展示了稳健的结果。

Conclusion: EpisTwin为可信赖的个人AI提供了有前景的方向，通过神经符号框架将生成推理建立在可验证的个人知识图谱基础上，解决了数据碎片化和语义理解不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 递归自我改进正从理论走向实践，但迭代自我修改存在微妙的对齐漂移风险，需要实用的框架来监控和控制这种漂移。

Method: 提出SAHOO框架，包含三个安全机制：(1)目标漂移指数(GDI)，结合语义、词汇、结构和分布度量的多信号检测器；(2)约束保持检查，强制执行安全关键不变性；(3)回归风险量化，标记可能撤销先前增益的改进周期。

Result: 在189个任务（代码生成、数学推理、真实性）中，SAHOO实现显著质量提升：代码任务提升18.3%，推理任务提升16.8%，同时在两个领域保持约束，真实性任务违规率低。阈值在18个任务的验证集上校准。

Conclusion: SAHOO使递归自我改进期间的对齐保持变得可测量、可部署和可系统验证，揭示了能力-对齐前沿：早期改进周期高效但后期对齐成本上升，以及领域特定的权衡（如流畅性与事实性）。
...
TL;DR: 该论文提出了一种schema-gated orchestration架构，通过将模式作为强制执行的边界，在保持对话灵活性的同时确保科学工作流的确定性、可追溯性和治理能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以将自然语言目标转化为可执行计算，但科学工作流需要确定性、可追溯性和治理能力，这在LLM自主决定执行内容时难以保证。研究发现工业研发中存在两个相互冲突的需求：确定性约束执行与对话灵活性。

Method: 通过18位专家的半结构化访谈识别需求，提出schema-gated orchestration作为解决方案。将需求操作化为执行确定性(ED)和对话灵活性(CF)，使用多模型LLM评分协议评估20个系统，涵盖5个架构组。

Result: 多模型LLM评分显示出高度一致性(ED: α=0.80, CF: α=0.98)，证明可作为人类专家评估的替代方案。系统评估揭示了一个经验帕累托前沿，没有系统同时实现高灵活性和高确定性，但在生成式和流程中心化极端之间出现了收敛区域。

Conclusion: schema-gated架构通过分离对话和执行权限，有望解耦灵活性与确定性的权衡。提出了三个操作原则：执行前澄清、约束性计划-执行编排、工具到工作流级别的门控，以指导实际采用。
